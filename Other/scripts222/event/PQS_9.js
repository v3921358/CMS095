...
var map;
function init() {
	eim = em.newInstance("PQS_9");
    map = eim.getMapInstance(932200200);
	ResetProperty();
	newtime();
	scheduleNew();
}
...
function scheduleNew() {
	...
	if (hour < 21){
		...
		em.broadcastServerMsg(5121033,"[家族跑旗活动] 活动将在"+date+"开始，希望大家积极参加。",true);
		em.setProperty("rank", "0");
		em.setProperty("gate", "0");
		em.setProperty("time", "0");
	}else if(hour==21 && min==00 && second==00){
		setupTask = em.scheduleAtTimestamp("newopen", 1000);
		em.broadcastServerMsg(5121033, "[家族跑旗活动] 活动开始，希望大家积极参加。" ,true);
		em.setProperty("rank", "0");
		em.setProperty("gate", "0");
		em.setProperty("time", "0");
	}else{
...
function start(eim) {
	...
	if (em.getProperty("gate")=="0"||em.getProperty("gate")==null){//0代表没有开启状态
		...
		if (em.getMapFactoryMap(932200200).getCharactersSize() < 5&&em.getProperty("gate")=="1") {
			map.startMapEffect("目前活动少于5人，请等待！", 5121050);
...
/*function overgame(){
	for (var i = 0; i < eim.getMapFactoryMap(932200200).getCharactersSize(); i++) {
		if (eim.getMapFactoryMap(932200200).getCharactersSize() != 0) {
			eim.getMapFactoryMap(932200200).getCharactersThreadsafe().get(i).getChar().warp(932200004, 0);//传送到定点上
		}
	}
	eim.getMapInstance(932200004).startMapEffect("请玩家在下次活动开启前领取奖励！否则无奖励", 5121050);
...
function scheduledTimeout(eim) {
    ...
    var map = eim.getMapInstance(932200200);
	if(em.getProperty("time")=="1"){
