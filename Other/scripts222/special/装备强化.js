    ...
    if (status == 0) {
		...
		if (选择的装备 >= 1 && 选择的装备 <= 96) {
			...
			var selStr = "";
			 ...
			 selStr += "#r下一级需要消耗: "+(获取选择的装备星级*3+3)+"个#z4031344##v4031344#\r\n"
			  selStr += "#r      需要消耗: 3000点卷 \r\n"
			
			 selStr += "#e#b#L999#开始强化装备#l     #L1000#退出强化系统#l\r\n "
				cm.sendSimple(selStr);
			} else {
				cm.sendOk("#r装备已满星 可选择进阶...\r\n#b若进阶成功可继续强化\r\n若进阶失败 星级清空 属性减少！");
				cm.dispose();
			}
		}
		if (选择的装备 == 99) {
		...
		if (获取选择的装备.getHpR() == 10){
			...
			var selStr = "";
			 ...
			 selStr += "#k当前属性: #r星级 "+获取选择的装备.getEnhance()+" 级 #r"+获取选择的装备.getOwner()+"\r\n"
			 selStr += "#k进阶成功: #r双攻+5点 属性保留#d(可继续进行强化)\r\n"
			 selStr += "#k进阶失败: #r属性-45点 星之力清空\r\n"
			 selStr += "#e#b当前成功率: #k"+成功率+"%(随级别变动)#n\r\n\r\n"
			 selStr += "#e#b需要消费30000点券#n\r\n\r\n"
			 selStr += "#e#r#L999#开始进阶装备#l     #L1000#退出进阶系统#l\r\n "
			 进阶 = 2;
				cm.sendSimple(selStr);
			} else {
				...
				cm.sendOk("#r只有当装备#b星之力为15星#r时才可进行进阶...");
				cm.dispose();
			}
		}
		if (选择的装备 == 99) {
		...
		if (cm.itemQuantity(4031344) >= (获取选择的装备星级*3+4)) {
			if (cm.getPlayer().getCSPoints(1) > 3000){
        		cm.gainNX(-3000);		
					...
					cm.playerMessage(1, "【装备星之力强化】\r\n\r\n恭喜您\r\n\r\n强化成功 星级:"+拷贝装备.getEnhance()+"星");
					cm.gainItem(4031344, -(获取选择的装备星级*3+3));}
					else {
                cm.sendOk("你的点卷不足3000.");
                cm.dispose();
            }
		} else {
		...
		拷贝装备.setWatk(拷贝装备.getWatk()+10);
		拷贝装备.setMatk(拷贝装备.getMatk()+10);
		拷贝装备.setHpR(拷贝装备.getHpR()+1);
		拷贝装备.setOwner("星之力-"+判定级别+"");
		cm.removeSlot(1, 选择的装备, 1);//消失的装备
		Packages.server.MapleInventoryManipulator.addFromDrop(cm.getC(), 拷贝装备, false);
		cm.playerMessage(1, "【装备星之力进阶】\r\n\r\n恭喜您\r\n\r\n进阶成功 级别:"+拷贝装备.getOwner()+"");
		} else {
			var 拷贝装备 = 获取选择的装备.copy();
			拷贝装备.setEnhance(0);//星级
