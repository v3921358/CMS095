...
function action(mode, type, selection) {
	if(cm.getPlayer().getMapId() != 0){
		...
		if (status == 0) {
			if (cm.getBossLog("答题重置" + 获取日期) == 0) {
			cm.setBossLog("答题重置" + 获取日期);
			cm.setBossRankCount9("答题总数"+获取日期+"",0);
			cm.setBossRankCount9("答题正确数"+获取日期+"",0);
			cm.sendOk("#b答题已重置 请重新打开NPC");
			cm.dispose();
			return;
			}
			var text = "我是饱读诗书的孔明，答题时间为每时#r50-55分#n#k,请准备：#b\r\n";
			text += "服务器当前时间：#r" + hour +":" + minute + ":" + second + "#n#k\r\n\r\n";
			text += Qicon + "今日答题总数: #r"+ cm.getBossRank9("答题总数"+获取日期+"", 2) +" #b道#n#k\r\n";
			text += Qicon + "今日正确题目: #r"+ cm.getBossRank9("答题正确数"+获取日期+"", 2) +" #b道#n#k\r\n";
			text += "#L1##b"+icon+" 点此查看答题详细规则（新手必看）#l\r\n";
			text += "#L2##b"+icon+" 开始答题（"+timeout+"秒限时，速度要快哦）#l\r\n";
			text += "#L3##r"+icon+" 答对题目数兑换 #b金币#l\r\n\r\n";
			text += "#b    <注意>:每天0点#r清空答题数据#b，请及时兑换\r\n";
			cm.sendSimple(text);
		}else if (status == 1) {
			if(selection==1){
					cm.sendOk("每次答题都有#b"+timeout+"秒钟时间限制#k，超过时间则失败，指定时间内回答错误也失败！点击开始答题后开始记时，在给出的题目中，#r一般有4个选项#k，请输入你认为正确的选项，点击确认即可完成答题.\r\n#r#e注意：每天的50分，#b如（1点50分至1点55分）#r,都可以答题，答题时间为5分钟。每次回答正确后，将会#d给你随机点卷奖励！");
					cm.dispose();
					return;
			}
			if(selection==2){
			...
			if(selection==3){
				typed = 2;
				var selText = "截至目前为止，您答对的题目总数为:#r"+cm.getBossRank9("答题正确数"+获取日期+"", 2) +"道#b\r\n";
				// selText += "#L1#" + icon + " 换取(抵用)(答对题目1道可换1000抵用)#l\r\n";
				selText += "#L3#" + icon + " 换取(金币)(答对题目1道可换20W金币)#l\r\n";
				...
				//selText += "\r\n#e#r注意：第一项换取魔方和卷轴后，不会减少答题数目，但后面的选项会扣掉相应的答题数目！\r\n\r\n";
				cm.sendSimple(selText);
			}
		}else if (status == 2) {
			if(typed == 1){
				cm.setBossRankCount9("答题总数"+获取日期+"",1);	
				...
				if(selection == tknow[5] || fee == tknow[5]){
					...
					if(randx >= 12 && randx<= 15){//征服币
						...
						cm.sendOk("恭喜，您答对了！#b正确答案:"+tknow[5]+"#k\r\n您获得了 " +randred+" 运动会币.");
						cm.worldSpouseMessage(0x20,"[答题系统]：恭喜 "+ cm.getChar().getName() +" 正确回答<诸葛孔明>的问题,得到 " +randred+" 运动会币~");
					}	*/				
				}else{
					cm.sendOk("很遗憾，回答错误！\r\n");
			...
			if(typed==2){
				if(selection==1){//换取6个魔方和3个防爆卷轴
					cm.dispose();
            cm.openNpc(2001003, "答题换抵用");
				} else if (selection == 2){//换取135级武器箱子
					if (cm.getBossRank9("答题正确数"+获取日期+"", 2) < 400){
					...
					cm.setBossRankCount9("答题正确数"+获取日期+"", -400);
					...
					cm.dispose();
            cm.openNpc(2001003, "答题换金币");
				} else if (selection == 4){//换取150级武器箱子
					if (cm.getBossRank9("答题正确数"+获取日期+"", 2) < 800){
					...
					cm.setBossRankCount9("答题正确数"+获取日期+"", -800);
