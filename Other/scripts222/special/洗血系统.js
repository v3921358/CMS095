...
function action(mode, type, selection) {
	...
	if (status == 0) {
		var selStr = "\t\t\t  " + 心 + " #r#e < 洗血系统 > #k#n  " + 心 + "\r\n\r\n";

		selStr += "		Hi~#b#h ##k，想提高自己的血量上限为之后的BOSS挑战提供更大的可能性么？#k\r\n\r\n";

		selStr += "#L0#" + kx+ "#e什么是洗血？" + kx + "\r\n\r\n";

		//selStr += "#L1#"+JD+"领取洗血智戒\r\n\r\n";

		//selStr += "#L2#"+JD+"快速洗血（洗蓝加血）\r\n";

		selStr += "#L3#" + JD + "使用点券兑换血量#l\r\n";
		/*if (cm.getPlayer().getBossLogD("洗血箱子") >= 20) {
			selStr += "#L4#"+JD+"购买#v" + 道具ID + "##z" + 道具ID + "#(每日限购50个)\r\n";
		}
		if (cm.getPlayer().getBossLogD("洗血箱子2") >= 50) {
			selStr += "#L5#"+JD+"购买#v" + 道具ID + "##z" + 道具ID + "#\r\n";
		}*/
		...
		if (selection == 0) {
			...
			cm.dispose();
			cm.openNpc(9900004, "洗血智戒");
		} else if (selection == 2) {
			cm.sendGetNumber("#e你想要洗多少次呢？#n\r\n\r\n快速洗血不可洗到#r最低MP#k以下哦！\r\n\r\n您当前的最大MP为:#b" + cm.getPlayer().getMaxMp() + "点#k\r\n最低可以洗到：#d" + minMp + "#k点", 1, 1, 1);
		} else if (selection == 3) {
			cm.sendGetText("#e你想使用点券兑换多少血量呢？\r\n当前比例" + persent + "点券:1血额外赠送*0.2%#n\r\n#b请输入血量:#k#l\r\n");
		}
		...
		if(cm.getPlayer().getCSPoints(1) <(persent*count)){
			cm.sendOk("点券不足！")
			cm.dispose();
			return;
		}
		cm.playerMessage(Number(cm.getPlayer().getMaxHp()) + count);
		if((cm.getPlayer().getMaxHp() + count)>99999){
			cm.sendOk("血量已达上限99999！")
			cm.dispose();
			return;
		}
		cm.gainNX(-persent*count);
		cm.getPlayer().setMaxHp((cm.getPlayer().getMaxHp() + count));
		cm.getPlayer().fakeRelog();
		cm.sendOk("恭喜你成功消耗"+persent*count+"点券增加了"+count+"*0.2点血量!");
		cm.dispose();
	}
}
