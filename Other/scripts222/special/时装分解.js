	...
	//	selStr += "#k请选择以下#d背包中的时装#k进行分解：\r\n\r\n\r\n";
		...
		if (选择的时装 >= 1 && 选择的时装 <= 96) {
			...
			var selStr = "";
			 ...
			 selStr += "#e#b#L999#开始分解时装#l     #L1000#退出分解系统#l\r\n "
				cm.sendSimple(selStr);
			} else {
				cm.sendOk("#r时装已满星 可选择分解...\r\n#b若分解成功可继续分解\r\n若分解失败 星级清空 属性减少！");
				cm.dispose();
			}
		}
		if (选择的时装 == 99) {
		...
		if (获取选择的时装.getHpR() == 10){
			...
			var selStr = "";
			 ...
			 selStr += "#k当前属性:#r"+获取选择的时装.getOwner()+"  =  "+获取选择的时装.getHpR()+" \r\n"
			 selStr += "#k分解可获得: #r#v4031344##z4031344#  "+获取选择的时装.getHpR()+" *400 #d(分解材料)\r\n\r\n\r\n"
			
			 selStr += "#e#r#L999#开始分解时装#l     #L1000#退出分解系统#l\r\n "
			 分解 = 2;
				cm.sendSimple(selStr);
			} else {
				...
				cm.sendOk("#r只有当时装有#b拥有级别#r才可进行分解...");
				cm.dispose();
			}
		}
		if (选择的时装 == 99) {
		...
		if (cm.itemQuantity(4031344) >= (获取选择的时装星级*3+4)) {
			if (cm.getPlayer().getCSPoints(1) > 5000){
        		cm.gainNX(-5000);		
					...
					cm.playerMessage(1, "【时装星之力分解】\r\n\r\n恭喜您\r\n\r\n分解成功 星级:"+拷贝时装.getEnhance()+"星");
		...
		var 拷贝时装 = 获取选择的时装.copy();
		 ...
		 cm.gainItem(4031344,获取选择的时装.getHpR()*400);
