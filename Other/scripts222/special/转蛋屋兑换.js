...
function action(mode, type, selection) {
    if (mode === -1) {
        ...
        if (status === 0) {
			...
			if (cm.getBossRankCount("转蛋屋积分") >= seletionData[1]) {
				cm.gainItem(seletionData[0],1);
				cm.setBossRankCount("转蛋屋积分", -seletionData[1]);
				cm.sendOk("恭喜兑换成功，获得 #v"+seletionData[0]+"##z"+seletionData[0]+"##r * 1");
				cm.dispose();
			} else {
				cm.sendOk("转蛋屋积分不足，无法兑换。");
				cm.dispose();
			}
        }
    }
 ...
 */
 function showMenu() {
    var text ="\t\t" + hwx + "#r#e < 转蛋屋积分兑换 > #k#n" + hwx + "\r\n\r\n\r\n";

	text+="当前有:"+cm.getBossRankCount("转蛋屋积分")+" 点转蛋屋积分"+rn;
