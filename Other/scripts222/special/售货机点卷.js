...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status === 0) {
            ...
            var add = "请选择你取走的物品, 当前点卷:#b" + cm.getPlayer().getCSPoints(1) + "#l" + rn;
            ...
            if (selType == 0) {
                ...
                if (cm.getPlayer().getCSPoints(1) >= selArr[selection][0]) {
                    txt2 = "#d确定使用【" + selArr[selection][0] + "】点卷取走#v" + selArr[selection][1] + "##z" + selArr[selection][1] + "# X " + selArr[selection][2] + " 个吗？#l";
                } else {
                    cm.sendOk("你投了个假币~~~");
                    cm.dispose();
                    return;
