...
function start() {
    ...
    if (cm.isGMS()) {
        ...
        pqMaps = Array(541020610, 310000000, 610030010, 931000310, 931000311, 931000312, 541010040, 541010050, 801040004, 240040000, 211060100, 211060300, 211060500, 211060700, 211060900, 551030100, 211042400, 211042401, 240050400, 921140001, 270050200, 271040000);
    } else {
        ...
        pqMaps = Array(541020610, 610030010, 931000310, 931000311, 931000312, 541010040, 541010050, 801040004, 240040000, 211060100, 211060300, 211060500, 211060700, 211060900, 551030100, 211042400, 211042401, 240050400, 921140001, 270050200, 271040000);
    }
}
    ...
    if (status == 0) {
        if (!cm.isQuestFinished(29003) && !cm.haveItem(1142184, 1, true, true)) {
            ...
            if (cm.canHold(1142184, 1)) {
                ...
                cm.sendOk("Welcome to ParadiseSEA! I congratulate you on getting to level 10. There is still a long way to go, Here are some items to help you on your journey with us!");
            } else {
        ...
        cm.sendSimple("Hello Agent #r#h ##k. \r\n#b#b#L3#I want to go somewhere.#l\r\n#L2##bLearn a Skill#l#k\r\n#L12##bChange your Look.#l#k\r\n#L16##bVote points NPC.#k#l\r\n#L9998##bDonation NPC#k#l\r\n#L152##bLegends Shop#k#l\r\n#L9999##bUniversal Shop#k#l\r\n#L9119##bJob Changer#k#l\r\n#L9685##bJaguar that produces cubes#k#l\r\n");
    } else if (status == 1) {
        if (selection == 12) {
            cm.dispose();
            cm.openNpc(9201117);
            ...
            cm.openNpc(9201094);
            return;
        } else if (selection == 9685) {
            cm.dispose();
            cm.openNpc(2159345);
            return;
        } else if (selection == 9119) {
            cm.dispose();
            cm.openNpc(1002010);
            return;
        ...
        if (selection == 1) {
            ...
            if (cm.getPlayer().getSkillLevel(80001000) > 0 || cm.getSkillLevel(cm.getPlayer(), 1004, cm.getPlayer().getJob())) {
                ...
                cm.teachSkill(cm.isGMS() ? 80001000 : cm.getSkillByJob(cm.getPlayer(), 1016, cm.getPlayer().getJob()), 1, 0); // Maker
