...
function action(mode, type, selection) {
    ...
    if (status == 0 && mode == 1) {
        if (cm.isQuestActive(3821)) {
            ...
            cm.sendNext("给~这是蟠桃果园的名药！");
            cm.dispose();
            return;
        }
        var selStr = "我是个多才多藝的人。跟我说说你想要什么東西。 #b"
        var options = new Array("制药", "制造卷軸");
        ...
        if (selectedType == 0) { //Make a medicine
            cm.sendNext("如果你想學做药，你第一步就是學習中药配方，沒有什么比这个更適合了。");
            cm.dispose();
            return;
            ...
            selStr = "什么？你想把所有的药材捐獻出來？真是好消息啊！捐獻道具是以#b100个#k为单位。給捐獻者可以制造符咒的魔珠。你細想捐獻那种药材？ #b";
        ...
        if (selectedType == 1) { //Scrolls
            ...
            item = itemSet[selectedItem];
            var prompt = "你确定以想要贊助 #b100个 #t " + item + "##k";
            cm.sendYesNo(prompt);
        }
