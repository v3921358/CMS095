		...
		if (status == 0) {
			...
			//	cm.removeAll(4001008);
				...
				if (mapId == 地图1 || mapId == 地图2) {
					if (cm.getBossRank9("橙金Boss", 2) < 怪物.length) {
						if (cm.getPlayer().getMap().mobCount() == 0) {
							if (cm.getBossRank9("橙金Boss", 2) <= 0) {
								cm.spawnMob(怪物[0], 1, -165, 34);
								cm.setBossRankCount9("橙金Boss", 1);
								cm.dispose();
							} else {
								cm.setBossRankCount9("橙金点", cm.getBossRank9("橙金Boss", 2));
								cm.playerMessage(6, "恭喜您，抗过第" + cm.getBossRank9("橙金Boss", 2) + "关 您获得" + cm.getBossRank9("橙金Boss", 2) + "点橙金点，总共" + cm.getBossRank9("橙金点", 2) + "点，接下来的Boss更强大！");
								cm.spawnMob(怪物[cm.getBossRank9("橙金Boss", 2)], 1, -165, 34);
								cm.setBossRankCount9("橙金Boss", 1);
								cm.dispose();
							}
						} else {
			...
			if (二级链接 == 1) {
		        ...
		        cm.warp(555000000, 0);
				cm.dispose();
				cm.setBossRankCount9("橙金Boss", -cm.getBossRank9("橙金Boss", 2));
			}
			if (二级链接 == 2) {
				cm.dispose();
				cm.gainItem(4001231, 6);
				cm.warp(555000000, 0);
				cm.setBossRankCount9("橙金点", 4);
				cm.playerMessage(6, "恭喜你完成所的Boss关卡，额外获得橙金点！总共" + cm.getBossRank9("橙金点", 2) + "点");
				cm.setBossRankCount9("橙金Boss", -cm.getBossRank9("橙金Boss", 2));
			}
		}
	}
