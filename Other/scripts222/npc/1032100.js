...
function action(mode, type, selection) {
	...
	cm.sendNext("这个 " + item + " 不好制作，准备好材料再來找我。");
	cm.dispose();
	return;
    }
    if (mode == 1)
	...
	if (cm.getPlayerStat("LVL") >= 40) {
	    cm.sendNext("是啊...我是仙女的炼金大师。但是，仙女不應該在一个人的时间很长一段时间的接觸......。如果你得到了我的資料，我会让你成为一个特殊的道具。");
	} else {
	    cm.sendOk("我可以做出稀有，貴重物品，但不幸的是，我不能让它像你这樣的陌生人。");
	    cm.dispose();
	}
    } else if (status == 1) {
