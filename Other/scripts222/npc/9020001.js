...
var chatState;
var questions = Array(
        "请问PP是不是最帅的管理员,1张代表是,10张代表不是",
        "请问PP冒险岛095是不是最长久稳定的服务器,2张代表是,20张代表不是",
        "请问PP冒险岛095的群号第一位数字是几",
        "请问管理员PP的QQ号第一位数字是几",
        "请问PP冒险岛095是星期几开始公测的",
        "请问PP冒险岛095是几月份开始公测的");
var qanswers = Array(1, 2, 4, 4, 2, 6);
var party;
var preamble;
var stage2combos = Array(Array(1, 1, 1, 0), Array(1, 1, 0, 1), Array(1, 0, 1, 1), Array(0, 1, 1, 1));
var stage3combos = Array(Array(1, 1, 1, 0, 0), Array(1, 1, 0, 1, 0), Array(1, 0, 1, 1, 0), Array(0, 1, 1, 1, 0), Array(1, 1, 0, 0, 1), Array(1, 0, 1, 0, 1), Array(0, 1, 1, 0, 1), Array(1, 0, 0, 1, 1), Array(0, 1, 0, 1, 1), Array(0, 0, 1, 1, 1));
        ...
        if (playerStatus) { // Check if player is leader
            ...
            if (status == 0 && preamble == null) {
                ...
                if (question == null) {
                    ...
                    var questionNum = cm.getDoubleFloor(cm.getDoubleRandom() * questions.length);
                ...
                var complete = eim.getProperty(curMap.toString() + "stageclear");
                if (complete != null) {
                    cm.sendNext("恭喜,你们成功的通过这阶段。现在即将达到下一阶段!");
                    cm.dispose();
                } else {
...
function rectanglestages(cm) {
    ...
    if (playerStatus) { // Check if player is leader
        if (status == 0) {
            ...
            if (preamble == null) {
                cm.sendNext("欢迎来到第 " + nthtext + " 阶段。 这阶段需要成员3个人找出答案才能过关,赶紧抓紧时间。");
                eim.setProperty("leader" + nthtext + "preamble", "done");
                var sequenceNum = cm.getDoubleFloor(cm.getDoubleRandom() * curcombo.length);
                ...
                if (complete != null) {
                    ...
                    if (totplayers == 3) {
                        ...
                        if (testcombo) {
                            // Do clear
                            clear(curMap, eim, cm);
                            var exp = (cm.getDoublePow(2, curMap) * 50) * 2;
...
function getPrize(eim, cm) {
    var itemSetSel = cm.getDoubleRandom();
    var itemSet;
    var itemSetQty;
    var hasQty = false;
    if (itemSetSel < 0.3)
    ...
    var sel = cm.getDoubleFloor(cm.getDoubleRandom() * itemSet.length);
    var qty = 1;
    if (hasQty)
        qty = itemSetQty[sel];
