...
function action(mode, type, selection) {
	...
	cm.sendNext("嘿，得到了一點點时间？好了，我的工作就是在这里收集物品和其他地方出售，但这些天怪物变得更加敵对的，所以它很难获得良好的道具...你怎么看？你想要做的一些業務和我在一起？");
    } else if (status == 1) {
	cm.sendYesNo("这筆交易很简单，首先你得得到我需要的東西，当然我会給你不错的酬勞。");
    } else if (status == 2) {
	...
	cm.sendYesNo("让我看看，你想要交換你的 #b100个 #t" + requiredItem + "##k 來換取一些酬勞是吧？ 交易前确保你的道具栏位是否足够吧！！");
    }else if (status == 4){
	itemSet = (cm.getDoubleFloor(cm.getDoubleRandom() * eQuestPrizes[lastSelection].length));
	reward = eQuestPrizes[lastSelection];
	prizeItem = reward[itemSet][0];
	prizeQuantity = reward[itemSet][1];
	
	if(!cm.haveItem(requiredItem,100)){
	    cm.sendOk("嗯... 你确定你有 #b100个 #t" + requiredItem + "##k? 如果有请定你道具攔是不是满了....");
		cm.dispose();
		return;
	} else if(!cm.canHold(prizeItem) || !cm.canHold(1302000) || !cm.canHold(2000006) || !cm.canHold(4000000)){
	    cm.sendNext("你的道具攔似乎满了，请清空一些不要的東西再來找我交易一次謝謝。");
		cm.dispose();
		return;
	}
