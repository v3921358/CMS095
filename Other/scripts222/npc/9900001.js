...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        statsSel = selection;
        if (selection == 24) {
            cm.sendGetText("你想要調整 #b#t" + cm.getInventory(-1).getItem(slot[selected]).getItemId() + "##k's 2" + stats[statsSel] + " 吗?");
        } else {
            cm.sendGetNumber("你想要調整 #b#t" + cm.getInventory(-1).getItem(slot[selected]).getItemId() + "##k's 1" + stats[statsSel] + " 吗?", 0, 0, 60004);
        }
