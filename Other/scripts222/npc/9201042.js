...
function start() {
    ...
    if (cm.getPlayer().getMapId() != 123123123) {
	cm.sendYesNo("Welcome to the Donation Party Quest! You only can go SOLO. But please be informed that the time given inside will be 1 minute long. Do you want to enter the Donation Hunting Map for a Red Luck Sacks?");
	return;
    }
	...
	while (it.hasNext()) {
	    var cPlayer = it.next();
	    if ((cPlayer.getLevel() >= 20 && cPlayer.getLevel() <= 255 && cm.getPlayer().getPoints() >= 2000) || cPlayer.getJobId() == 900) {
		levelValid += 1;
	    } else {
		next = false;
	    }
...
function action(mode, type, selection) {
	...
	} else if (cm.haveItem(3993003,1)) {
	    cm.gainItem(3993003, -1); // PQ Handler Script
	    cm.warp(502050600,1);
	} else {
	    cm.sendOk("You either do not have a Red Luck Sack or You need to be alone.");
	}
	}
 	    cm.dispose();
}