...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {	
            cm.sendNext("嘿,我是#p" + cm.getNpc() + "#, 如果你不忙，所有的…那么我可以和你一起出去玩吗？我听说有人聚集在这里附近的一个 #r活动#k 但我不想自己去那里…嗯，你想和我一起去看看吗?");
		} else if (status == 1) {	
            cm.sendSimple("嗯？什么样的活动？嗯，那是...\r\n#L0##e1.#n#b 它是一个什么样的活动?#k#l\r\n#L1##e2.#n#b 向我解释活动游戏.#k#l\r\n#L2##e3.#n#b 好吧，我们走吧!#k#l\r\n#L3##e4.#n#b请更换奖励项连胜证书.#k#l");
		} else if (status == 2) {
			if (selection == 0) {
				...
				if (dat + 3600000 >= cm.getCurrentTime()) {
                    ...
                    cm.sendNext("要么活动尚未启动，要么您已经参加了这一活动。请稍后再试!");
				}
				cm.dispose();
			} else if (selection == 3) {
			...
			var rand = cm.getDoubleFloor(cm.getDoubleRandom() * pri.length);
