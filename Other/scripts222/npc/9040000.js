...
function action(mode, type, selection) {
	...
	if (selection == 0) { //Start
		...
		cm.sendNext("只有公会长和副会长才能啟动公会守护战。");
		cm.dispose();
	    } else {
		var em = cm.getEventManager("GuildQuest");
		if (em == null) {
		    cm.sendOk("公会守护战尚未实施建設。");
		} else {
			...
			cm.guildMessage("公会已经开始公会守护战 <頻道:" + cm.getClient().getChannel() + "> 请公会成员盡速來到遺跡发掘对營地參加公会守护战。");
		    } else {
		...
		if (em == null) {
			...
			//} else 
			    ...
			    cm.sendOk("我很抱歉，但公会已经沒有你，请稍后再嘗试！");
			}
		    }
		}
		cm.dispose();
