    ...
    switch (status) {
        case 0:
            if (cm.getPlayer().getLevel() <= 29) {
                cm.sendOk("潮流轉蛋機只能從30級开始使用。");
                status = -1;
                //cm.gainItem(5220000, 1);
                cm.dispose();
                break;
            } else {
                cm.sendYesNo("你好，我是潮流轉蛋，非常潮哦！有多潮看下面说明\r\n#L0#點我轉蛋!#l"+/*"\r\n#L3##r十連抽送一抽（十一抽）!#k#l"+*/"\r\n#L1#點我查看此轉蛋機內容!#l\r\n" + (cm.getPlayer().isGM() ? "#L2#點我更改此轉蛋機內容!(GM顯示功能)#l" : ""));
                break;
            }
        case 1:
            {
                if (selection == 1) {
                    ...
                    if (!cm.haveItem(requireItem, 10)) {
                        cm.sendOk("很抱歉由于你沒有#b#i" + requireItem + "##kx10，所以不能轉蛋哦。");
                        cm.dispose();
                        return;
                    }
                    if (!cm.canHoldByType(1, 11)) {
                    ...
                    if (!cm.canHoldByType(3, 11)) {
                        cm.sendOk("请确认背包是否已经满了。");
                        cm.dispose();
                        return;
                    }
                    if (!cm.canHoldByType(4, 11)) {
                        cm.sendOk("请确认背包是否已经满了。");
                        cm.dispose();
                        return;
                    }
                    if (!cm.canHoldByType(5, 11)) {
                        cm.sendOk("请确认背包是否已经满了。");
                        cm.dispose();
                        return;
                    }
                    for (var i = 0; i < 11; i++) {
                        ...
                        //} else {
                        //   cm.sendOk("很抱歉由于你沒有#b#i" + requireItem + "##k，所以不能轉蛋哦。");
                        //   cm.dispose();
                        //}
                    }
                } else if (selection == 0) {
