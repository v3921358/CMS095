...
public class MapleSquad {
    ...
    public static enum MapleSquadType {
        bossbalrog(1), zak(1), chaoszak(1), horntail(1), chaosht(1), pinkbean(1), nmm_squad(1), vergamot(1), dunas(1), nibergen_squad(1), dunas2(1), core_blaze(1), aufheben(1), cwkpq(1), tokyo_2095(1), vonleon(1), scartar(1), cygnus(1), hilla(1), darkhilla(1), arkarium(1), bossbloody(1), bosspierre(1), bossbanban(1), bossbelen(1), bossmagnus(1), zhilla(1);
        ...
        while (type.queue.get(ch).size() > 0 && ChannelServer.getInstance(ch).getMapleSquad(type) == null) {
            ...
            if (theirCh > 0) {
                ...
                if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
                    ...
                    if (lead != null) {
                        lead.dropMessage(6, "远征队已经結束了，由于沒有在正确的頻道裡。");
                    }
                    getBeginMap().broadcastMessage(MaplePacketCreator.serverNotice(6, nextPlayerId + "远征队已经結束了，由于有成員沒有在地图內"));
                    type.queuedPlayers.get(ch).add(new Pair<String, String>(nextPlayerId, "Not in map"));
                }
            } else {
                getBeginMap().broadcastMessage(MaplePacketCreator.serverNotice(6, nextPlayerId + "远征队已经結束了，由于有成員沒有在线上"));
    ...
    public String getNextPlayer() {
        ...
        sb.append("你是否想要 #e當下一个#n 在远征队排隊中　或者 #e移除#n 在远征队? 如果你想的话...");
        return sb.toString();
    }
    ...
    public String getSquadMemberString(byte type) {
        switch (type) {
            case 0: {
                StringBuilder sb = new StringBuilder("目前远征队總人數 : ");
