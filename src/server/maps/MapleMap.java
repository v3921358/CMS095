...
public final class MapleMap {
    ...
    private short decHP = 0, createMobInterval = 5000, top = 0, bottom = 0, left = 0, right = 0;
    private int consumeItemCoolTime = 0, protectItem = 0, decHPInterval = 10000, returnMapId, timeLimit,
    ...
    private List<String> chrname = new LinkedList<String>();
    ...
    private void spawnAndAddRangedMapObject(final MapleMapObject mapobject, final DelayedPacketCreation packetbakery) {
        ...
        try {
            ...
            while (itr.hasNext()) {
                chr = itr.next();
                if (!chr.isClone() && (mapobject.getType() == MapleMapObjectType.MIST || chr.getTruePosition().distance(mapobject.getTruePosition()) <= GameConstants.maxViewRangeSq())) {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            //System.out.println("id:"+de.itemId+ " name:"+MapleItemInformationProvider.getInstance().getName(de.itemId));
            if (de.itemId == mob.getStolen()) {
                continue;
            }
            if (Randomizer.nextInt(999999) < (int) (de.chance * chServerrate * chr.getDropMod() * chr.getDropm() * (chr.getStat().dropBuff / 100.0) * (showdown / 100.0))) {
                ...
                if (de.itemId == 0) { // meso
                    ...
                    if (mesos > 0) {
                        spawnMobMesoDrop((int) (mesos * (chr.getStat().mesoBuff / 100.0) * chr.getDropMod() * cmServerrate / 10), calcDropPos(pos, mob.getTruePosition()), mob, chr, false, droptype);
                        mesoDropped = true;
                    }
                } else {
        ...
        if (Randomizer.nextInt(100) <= 80) {
            ...
            if (mesos > 0) {
                ...
                spawnMobMesoDrop((int) (mesos * chr.getDropMod() * chr.getDropm() * cmServerrate / 10), calcDropPos(pos, mob.getTruePosition()), mob, chr, false, droptype);
            }
        }
        ...
        /*if (Randomizer.nextInt(100) <= 10) {
            ...
            int csz = 10;//a[(int) (Math.random() * a.length)];
            int size = chr.getAcLogD("怪物点數");
            int sizea = 500 - size;
            if (size < 500 && size >= 0) {
                chr.setAcLogS("怪物点數");
                chr.modifyCSPoints(2, csz, true);
                //chr.dropMessage(-1, "恭喜你通过打怪获得点數" + csz + "還能获取" + sizea + "次。");
                chr.dropMessage(-1, "恭喜你通过打怪获得点數" + csz);
            }
        }*/
    }
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
        ...
        switch (monster.getId()) {
            case 8800002://殘暴炎魔
                if (mapid == 280030000) {
                    for (MapleCharacter map : chr.getMap().getCharactersThreadsafe()) {
                        map.setBossLog("普炎攻打次數");
                    }
                }
                break;
            case 8800102://混沌殘暴炎魔
            ...
            case 8810122: //混沌闇黑龍王
                if (mapid == 240060201) {
                    for (MapleCharacter map : chr.getMap().getCharactersThreadsafe()) {
                        map.setBossLog("混龍攻打次數");
                    }
                }
                break;
            case 9420549:// 狂暴的娃娃獅王
                if (mapid == 551030200) {
                    for (MapleCharacter map : chr.getMap().getCharactersThreadsafe()) {
                        map.setBossLog("娃娃獅攻打次數");
                    }
                }
                break;
            case 9420544:// 狂暴的泰勒熊
            ...
            case 8840000: //凡雷恩
                if (mapid == 211070100) {
                    for (MapleCharacter map : chr.getMap().getCharactersThreadsafe()) {
                        map.setBossLog("凡雷恩攻打次數");
                    }
                }
                break;
            case 8850011: //西格諾斯 女皇
                if (mapid == 271040100) {
                    for (MapleCharacter map : chr.getMap().getCharactersThreadsafe()) {
                        map.setBossLog("女皇攻打次數");
                    }
                }
                break;
            default:
        ...
        if (chr.getBossRank("赏金任务", (byte) 1) > 0) {
            if (chr.getBossRank("赏金任务", (byte) 1) == mobid) {
                int count = chr.getBossRank("赏金任务", (byte) 2);
                chr.setBossRank("赏金任务", (byte) 2, 1);
            }
        }
        if (mobid == 8810018 && mapid == 240060200) { // Horntail
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "经过无數次的挑战，" + chr.getName() + "所帶領的队伍終于击败了暗黑龙王的远征队！你們才是龍之林的真正英雄~"));
            speedRunStart = chr.getStartTime();
            speedRunLeader = chr.getName();
            chr.setStartTime(0);

            for (MapleCharacter mapchr : chr.getMap().getCharactersThreadsafe()) {
                if (!mapchr.isIntern()) {
                    chrname.add(mapchr.getName());
                }
            }
            if (speedRunStart > 0) {
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "经过无數次的挑战，" + chr.getName() + "所帶領的队伍終于擊破了混沌暗黑龙王的远征队！你們才是龍之林的真正英雄~"));
            speedRunStart = chr.getStartTime();
            speedRunLeader = chr.getName();
            chr.setStartTime(0);
            for (MapleCharacter mapchr : chr.getMap().getCharactersThreadsafe()) {
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "经过无數次的挑战，" + chr.getName() + "所带领的队伍終于終于击败了時间的宠儿－品克缤的远征队！你們才是時间神殿的真正英雄~"));
            if (speedRunStart > 0) {
                type = ExpeditionType.Pink_Bean;
            }
            doShrine(true);
        } else if (mobid == 8850011 && mapid == 271040100) {// msea map id i guess.
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "被黑魔法師黑化的女皇終于被" + chr.getName() + "所帶領的永不言敗的远征队打倒!混沌世界得以淨化!"));
            if (speedRunStart > 0) {
                type = ExpeditionType.Cygnus;
            }
            doShrine(true);
        } else if (mobid == 8870000 && mapid == 262030300) { //hilla
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "Hilla is dead."));

            if (speedRunStart > 0) {
                type = ExpeditionType.Hilla;
            }
            doShrine(true);
        } else if (mobid == 8880600 && mapid == 262000200) { //Zhilla
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "ZHilla is dead."));

            if (speedRunStart > 0) {
                type = ExpeditionType.ZHilla;
            }
            doShrine(true);
        } else if (mobid == 8870100 && mapid == 262031300) { //hilla
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "Dark Hilla is dead."));
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
        ...
        if (type != null) {
            if (speedRunStart > 0 && speedRunLeader.length() > 0) {
                ...
                broadcastMessage(MaplePacketCreator.serverNotice(5, speedRunLeader + " 远征队花了 " + time + " 時间打敗了 " + SpeedRunner.getTypeName(type.name()) + "!"));
                getRankAndAdd(speedRunLeader, time, type, (endTime - speedRunStart), (sqd == null ? (chrname.isEmpty() ? null : chrname) : sqd.getMembers()));
                chrname.clear();
                endSpeedRun();
            }
    ...
    public void removeSummon(final int summonid) {
        ...
        this.broadcastMessage(MaplePacketCreator.removeSummon(summon, false));
    }
    ...
    public final void spawnMobMesoDrop(final int meso, final Point position, final MapleMapObject dropper, final MapleCharacter owner, final boolean playerDrop, final byte droptype) {
        final MapleMapItem mdrop = new MapleMapItem(meso < 1 ? 1 : meso, position, dropper, owner, droptype, playerDrop);
...
//	broadcastMessage(MaplePacketCreator.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 0));
    ...
    public final void addPlayer(final MapleCharacter chr) {
            ...
            if (LoginServer.getChrPos() != null) {
                for (Map.Entry<Integer, Pair<Integer, Integer>> ms : LoginServer.getChrPos().entrySet()) {
                    ...
                    if (ch == -1) {
                        ...
                        chr.getClient().getSession().write(MaplePacketCreator.spawnPlayerMapobject(victim));
                    }
                }
            }
        }
        ...
        if (chr.getAndroid() != null && chr.checkHearts()) {
        ...
        //chr.getClient().getSession().write(MaplePacketCreator.getPublicNPCInfo());
        //}
        if (!chr.isClone()) {
        ...
        if (!chr.isClone()) {
            ...
            if (mapid != 105200510 && mapid != 105200610 && mapid != 105200710 && mapid != 105200810 && mapid != 401060100 && mapid != 262030300 && mapid != 272020200 && mapid != 262000200) {
        ...
        }*/ //temporary remove? i doesn't need this stupid thing.
    ...
    public final MapleSquad getSquadByMap() {
        ...
        switch (mapid) {
            ...
            case 272030400:
                ...
                break;
            case 105200710:
                zz = MapleSquadType.bossbloody;
                break;
            case 105200610:
                zz = MapleSquadType.bosspierre;
                break;
            case 105200510:
                zz = MapleSquadType.bossbanban;
                break;
            case 105200810:
                zz = MapleSquadType.bossbelen;
                break;
            case 401060100:
                zz = MapleSquadType.bossmagnus;
                break;
            default:
                return null;
        }
    ...
    public void respawn(final boolean force, final long now) {
        ...
        if (force) { //cpq quick hack
            ...
            //final int numShouldSpawn = monsterSpawn.size() * ServerConstants.monsterSpawn - spawnedMonstersOnMap.get();
            ...
            //final int numShouldSpawn = (GameConstants.isForceRespawn(mapid) ? monsterSpawn.size() * ServerConstants.monsterSpawn : maxRegularSpawn * ServerConstants.monsterSpawn) - spawnedMonstersOnMap.get();
            final int numShouldSpawn = (GameConstants.isForceRespawn(mapid) ? monsterSpawn.size() * MapConstants.isMonsterSpawn(this) : maxRegularSpawn * MapConstants.isMonsterSpawn(this)) - spawnedMonstersOnMap.get();
            if (numShouldSpawn > 0) {
                int spawned = 0;
                ...
                for (Spawns spawnPoint : randomSpawn) {
                    ...
                    //if (spawnPoint.shouldSpawn(lastSpawnTime) || GameConstants.isForceRespawn(mapid) || (monsterSpawn.size() * ServerConstants.monsterSpawn < 10 && maxRegularSpawn * ServerConstants.monsterSpawn > monsterSpawn.size() * ServerConstants.monsterSpawn && partyBonusRate > 0)) {
                    if (spawnPoint.shouldSpawn(lastSpawnTime) || GameConstants.isForceRespawn(mapid) || (monsterSpawn.size() * MapConstants.isMonsterSpawn(this) < 10 && maxRegularSpawn * MapConstants.isMonsterSpawn(this) > monsterSpawn.size() * MapConstants.isMonsterSpawn(this) && partyBonusRate > 0)) {
                        spawnPoint.spawnMonster(this);
                        spawned++;
                    }
                    if (spawned >= numShouldSpawn) {
            ...
            try {
                for (MapleCharacter chr : characters) {
                    if (chr.getName().equals(squad.getLeaderName()) && !chr.isIntern()) {
                        ...
                        chr.setStartTime(System.currentTimeMillis());
                        return;
                    }
                }
            } finally {
    ...
    public final boolean canSpawn(long now) {
        if (MapConstants.isSpawnSpeed(this)) {
            createMobInterval = 0;
        } else {
            createMobInterval = 5000;
        }
