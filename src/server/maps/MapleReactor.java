...
//        System.out.println("statstype:"+stats.getType(state));
...
//        System.out.println("charPos:"+charPos);
        if (stats.getType(state) < 999 && stats.getType(state) != -1) {
            ...
            if (!(stats.getType(state) == 2 && (charPos == 0 || charPos == 2)) || Arrays.asList(rids).contains(rid+"") ) { // next state 无法打击的反应堆，在顶部rids添加ID PPMS
