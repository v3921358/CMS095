...
public class MapScriptMethods {
    ...
    public static void startScript_FirstUser(MapleClient c, String scriptName) {
        ...
        if (c.getPlayer().isAdmin() || LoginServer.isLogPackets()) {
            c.getPlayer().dropMessage(6, "[系統提示]您已经建立与地图脚本:" + scriptName + "的连接。(FirstUser)" + " 地图ID: " + c.getPlayer().getMapId());
        }
        ...
        switch (onFirstUserEnter.fromString(scriptName)) {
            ...
            case party6weatherMsg: {
                switch (c.getPlayer().getMapId()) {
                    case 930000000:
                        c.getPlayer().getMap().startMapEffect("进入傳送点，我要对你們施放变身魔法了！", 5120023);
                        break;
                    case 930000100:
                    ...
                    case 930000300:
                        ...
                        break;
                    case 930000400:
                        c.getPlayer().getMap().startMapEffect("找我对话拿淨化之珠其中一个隊員集满10个怪物株給我！", 5120023);
                        break;
                    case 930000500:
            ...
            case kenta_mapEnter:
                ...
                break;
            case cygnus_Summon: {
                c.getPlayer().getMap().startMapEffect("已经好久沒有客人到來了. 也已经好久沒有人活著离开这了。", 5120043);
                break;
            }
            case iceman_Boss: {
    ...
    public static void startScript_User(final MapleClient c, String scriptName) {
        ...
        if (c.getPlayer().isAdmin() || LoginServer.isLogPackets()) {
            c.getPlayer().dropMessage(6, "[系統提示]您已经建立与地图脚本:" + scriptName + "的连接。(User)" + " 地图ID: " + c.getPlayer().getMapId());
        }
        String data = "";
        switch (onUserEnter.fromString(scriptName)) {
            ...
            case evanPromotion:
                ...
                break;
            case mPark_stageEff:
                c.getPlayer().dropMessage(-1, "要进入下一階段，必須消滅所有怪物。");
