...
public class MapleQuest implements Serializable {
    ...
    public void complete(MapleCharacter c, int npc, Integer selection) {
        ...
        if (id == 2342) {
            if (canHold(c)) {
                if (!c.haveItem(1142188, 1, false, true)) {
                    MapleInventoryManipulator.addById(c.getClient(), 1142188, (short) 1, "");
                }
            } else {
                c.dropMessage(1, "您的背包空间不足，无法完成任务。");
                return;
        ...
        if (c.getMap() != null && (autoPreComplete || checkNPCOnMap(c, npc)) && canComplete(c, npc)) {
            ...
            c.getClient().getSession().write(MaplePacketCreator.showSpecialEffect(0xC)); // Quest completion
            c.getMap().broadcastMessage(c, MaplePacketCreator.showSpecialEffect(c.getId(), 0xC), false);
        }
    }
    ...
    private boolean checkNPCOnMap(MapleCharacter player, int npcid) {
        //mir = 1013000
        return (GameConstants.isEvan(player.getJob()) && npcid == 1013000) || id == 3194 || npcid == 9000040 || npcid == 9000066 || (player.getMap() != null && player.getMap().containsNPC(npcid));
    }
    ...
    public static enum MedalQuest {
        ...
        奇幻村探險家(29014, 29015, 50, new int[]{105000000, 105000000, 105010100, 105020100, 105020300, 105030000, 105030100, 105030300, 105030500, 105030500}); //repeated map
