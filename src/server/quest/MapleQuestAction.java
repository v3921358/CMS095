        ...
        switch (type) {
            case exp:
                ...
                c.gainExp(intStore /**
                         ...
                         * ((c.getTrait(MapleTraitType.sense).getLevel() * 3 /
                         * 10) + 100) / 100
                         */
                        , true, true, true);
                break;
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // remove items
                        if (!c.haveItem(id, count, false, true)) {
                            c.dropMessage(1, "您是短一些項目來完成任务。");
                            return false;
                        }
                    } else { // add items
                        if (MapleItemInformationProvider.getInstance().isPickupRestricted(id) && c.haveItem(id, 1, true, false)) {
                            c.dropMessage(1, "你已经有了这个道具: " + MapleItemInformationProvider.getInstance().getName(id));
                            return false;
                        }
            ...
            case item: {
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // remove items
                        ...
                        if (quest.getId() == 8630) {
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM聊天] 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用10張枫彩票兌換物品:" + name + "ID:" + id));
                            FileoutputUtil.logToFile("logs/Data/枫彩票兌換.txt", "\r\n 時间　[" + FileoutputUtil.NowTime() + "] IP: " + c.getClient().getSessionIPAddress() + " 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用10張枫彩票兌換物品:" + name + "ID:" + id);
                        }
                        if (quest.getId() == 8631) {
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM聊天] 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用50張枫彩票兌換物品:" + name + "ID:" + id));
                            FileoutputUtil.logToFile("logs/Data/枫彩票兌換.txt", "\r\n 時间　[" + FileoutputUtil.NowTime() + "] IP: " + c.getClient().getSessionIPAddress() + " 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用50張枫彩票兌換物品:" + name + "ID:" + id);
                        }
                        if (quest.getId() == 8632) {
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM聊天] 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用100張枫彩票兌換物品:" + name + "ID:" + id));
                            FileoutputUtil.logToFile("logs/Data/枫彩票兌換.txt", "\r\n 時间　[" + FileoutputUtil.NowTime() + "] IP: " + c.getClient().getSessionIPAddress() + " 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用100張枫彩票兌換物品:" + name + "ID:" + id);
                        }
                        if (quest.getId() == 8633) {
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM聊天] 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用200張枫彩票兌換物品:" + name + "ID:" + id));
                            FileoutputUtil.logToFile("logs/Data/枫彩票兌換.txt", "\r\n 時间　[" + FileoutputUtil.NowTime() + "] IP: " + c.getClient().getSessionIPAddress() + " 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用200張枫彩票兌換物品:" + name + "ID:" + id);
                        }
                        if (quest.getId() == 8634) {
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM聊天] 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用500張枫彩票兌換物品:" + name + "ID:" + id));
                            FileoutputUtil.logToFile("logs/Data/枫彩票兌換.txt", "\r\n 時间　[" + FileoutputUtil.NowTime() + "] IP: " + c.getClient().getSessionIPAddress() + " 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用500張枫彩票兌換物品:" + name + "ID:" + id);
                        }
                        if (quest.getId() == 8635) {
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM聊天] 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用1000張枫彩票兌換物品:" + name + "ID:" + id));
                            FileoutputUtil.logToFile("logs/Data/枫彩票兌換.txt", "\r\n 時间　[" + FileoutputUtil.NowTime() + "] IP: " + c.getClient().getSessionIPAddress() + " 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用1000張枫彩票兌換物品:" + name + "ID:" + id);
                        }
                        if (quest.getId() == 8636) {
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM聊天] 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用3000張枫彩票兌換物品:" + name + "ID:" + id));
                            FileoutputUtil.logToFile("logs/Data/枫彩票兌換.txt", "\r\n 時间　[" + FileoutputUtil.NowTime() + "] IP: " + c.getClient().getSessionIPAddress() + " 任务ID:" + quest.getId() + " 玩家:" + c.getName() + " 使用3000張枫彩票兌換物品:" + name + "ID:" + id);
                        }
