    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (poison && getHp() > 1) {
            ...
            if (from.isAdmin()) {
                ...
                from.dropMessage(6, "[持续伤害] 加成 - 技能ID：" + eff.getDOT() + " 被动： " + from.getStat().dot + " 被动加成： " + damageIncrease + " 最终加成：" + poisonDot);
            }
            ...
            if (from.isAdmin()) {
                from.dropMessage(6, "[持续伤害] 持续伤害： " + ((getHp() > poisonDamage) ? poisonDamage : (getHp() - 1)) + " 持续時间：" + aniTime + " 持续掉血：" + status.getX());
            }
    ...
    public final void handleSteal(MapleCharacter chr) {
        ...
        if (level > 0 && !getStats().isBoss() && stolen == -1 && steal.getEffect(level).makeChanceResult()) {
            ...
            for (MonsterDropEntry d : dropEntry) { //set to 4x rate atm, 40% chance + 10x
                if (d.itemId > 0 && d.questid == 0 && d.itemId / 10000 != 238 && Randomizer.nextInt(999999) < (int) (10 * d.chance * chServerrate * chr.getDropMod() * chr.getDropm() * (chr.getStat().dropBuff / 100.0) * (showdown / 100.0))) { //kinda op
