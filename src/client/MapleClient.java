...
public class MapleClient implements Serializable {
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            if (rs.next()) {
                ...
                if (banned > 0 && !gm) {
                    ...
                    } else //boolean updatePasswordHash = false;
    ...
    public boolean CheckSecondPassword(String in) {
        boolean allow = false;
        if (salt2 == null && LoginCrypto.checkSha1Hash(secondPassword, in)) {
            allow = true;
        }
        return allow;
    }
            ...
            /*try {
                if (player.getBuffTimeCout(2450000) > 0) {
                    ...
                    if (startTime + length - System.currentTimeMillis() > 3000) {
                        player.updateBuffTime(2450000, (int) (startTime + length - System.currentTimeMillis()));
                    } else {
                ...
                if (player.getBuffTimeCout(2450010) > 0) {
                    ...
                    if (startTime + length - System.currentTimeMillis() > 3000) {
                        player.updateBuffTime(2450010, (int) (startTime + length - System.currentTimeMillis()));
                    } else {
    ...
    public final void disconnect(final boolean RemoveInChannelServer, final boolean fromCS, final boolean shutdown) {
        try {
            ...
            engines.clear();
        } catch (final Exception exse) {
            FileoutputUtil.outputFileError("logs/離线异常.txt", exse);
        }
    ...
    public final boolean CheckIPAddress() {
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            final PreparedStatement ps = con.prepareStatement("SELECT SessionIP, banned FROM accounts WHERE id = ?");
    ...
    public final int deleteCharacter(final int cid) {
        ...
        FileoutputUtil.logToFile("logs/Data/角色删除.txt", FileoutputUtil.NowTime() + " 账号: " + accountName + "(" + accId + ") 角色: " + cid + " (" + name + ") IP: " + getSessionIPAddress() + " \r\n");
...
//          MapleCharacter.deleteWhereCharacterId(con, "DELETE FROM cheatlog WHERE characterid = ?", cid);
    ...
    public boolean getbindac(int uid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement pss;
            pss = con.prepareStatement("SELECT login_expire, bind_line FROM accounts WHERE id = ?");
            pss.setInt(1, uid);
            try (ResultSet rs = pss.executeQuery()) {
                if (rs.next()) {
                    long time = rs.getLong("login_expire");
                    String lineID = rs.getString("bind_line");
                    long unix = System.currentTimeMillis() / 1000L;
                    if (lineID == null || (lineID != null && time > unix)) {
                        PreparedStatement ps;
                        ps = con.prepareStatement("UPDATE accounts SET login_expire = 0 WHERE id = ?");
                        ps.setInt(1, uid);
                        ps.execute();
                        ps.close();
                        return false;
    ...
    public static final byte getAcLogS(String charname, String bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            System.err.println("Error while getAcLogS" + e);
            FileoutputUtil.outError("logs/数据库异常.txt", e);
            return -2;
        }
        return 0;
    ...
    public static final byte getBossLogS(String charname, String bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            System.err.println("Error while getBossLogS" + e);
            FileoutputUtil.outError("logs/数据库异常.txt", e);
            return -2;
        }
        return 0;
