...
public class Equip extends Item implements Serializable {
    ...
    public Item reset(Equip newEquip) {
        ...
        this.hpR = newEquip.hpR;
        this.mpR = newEquip.mpR;
    ...
    public void resetPotential_Fuse(boolean half, int potentialState) { //equip first receive
        ...
        if (Randomizer.nextInt(100) < 4) {
            potentialState -= Randomizer.nextInt(100) < 4 ? (Randomizer.nextInt(100) < 4 ? 3 : 2) : 1;
        }
   ...
   public void resetPotential() { //equip first receive
        //0.16% chance unique, 4% chance epic, else rare
        final int rank = Randomizer.nextInt(1000) < 4 ? (Randomizer.nextInt(1000) < 4 ? -6 : -5) : -5;
    ...
    public void renewPotential(boolean prem) {
        ...
        switch (getState()) {
            case 5:
                rank = Randomizer.nextInt(1000) < LoginServer.getRenewPotentiaA() && getState() != 7 ? -(getState() + 1) : -(getState());
                break;
            case 6:
                rank = Randomizer.nextInt(1000) < LoginServer.getRenewPotentiaS() && getState() != 7 ? -(getState() + 1) : -(getState());
                break;
            default:
                rank = Randomizer.nextInt(1000) < 1 && getState() != 7 ? -(getState() + 1) : -(getState());
                break;
        }
        if (rank == -8) {
            rank = -7;
        ...
        setPotential1((short) rank);
        setPotential2((short) (getPotential3() > 0 || (prem && Randomizer.nextInt(100) < 2) ? rank : 0)); //1/10 chance of 3 line
        setPotential3((short) 0); //just set it theoretically
    }

    public void renewPotential1(boolean prem) {
        int rank = 0;
        switch (getState()) {
            case 5:
                rank = Randomizer.nextInt(1000) < LoginServer.getRenewPotentialsuperA() && getState() != 7 ? -(getState() + 1) : -(getState());
                break;
            case 6:
                rank = Randomizer.nextInt(1000) < LoginServer.getRenewPotentialsuperS() && getState() != 7 ? -(getState() + 1) : -(getState());
                break;
            default:
                rank = Randomizer.nextInt(1000) < 1 && getState() != 7 ? -(getState() + 1) : -(getState());
                break;
        ...
        setPotential1((short) rank);
        setPotential2((short) (getPotential3() > 0 || (prem && Randomizer.nextInt(1000) <  LoginServer.getRenewPotentialsuper3()) ? rank : 0)); //1/10 chance of 3 line
        setPotential3((short) 0); //just set it theoretically
    }

    public void renewPotential_super() {
        // 10% is too high....8% enough
        final int rank = Randomizer.nextInt(100) < 8 && getState() != 8 ? -(getState() + 1) : -(getState());
        setPotential1((short) rank);
    ...
    public void resetPotentialA() {
        final int rank = -6;
        setPotential1(rank);
        setPotential2(Randomizer.nextInt(10) <= 1 ? rank : 0); //1/10 chance of 3 line
    ...
    public byte getAddState() {
        byte ret = 0;
        if (this.potential4 >= 40000 || this.potential5 >= 40000 /*|| this.potential6 >= 40000*/) {
            ret = 8;
        } else if (this.potential4 >= 30000 || this.potential5 >= 30000 /*|| this.potential6 >= 30000*/) {
            ret = 7;
        } else if (this.potential4 >= 20000 || this.potential5 >= 20000 /*|| this.potential6 >= 20000*/) {
            ret = 6;
        } else if (this.potential4 >= 1 || this.potential5 >= 1 /*|| this.potential6 >= 1*/) {
            ret = 5;
        } else if (this.potential4 < 0 || this.potential5 < 0 /*|| this.potential6 < 0*/) {
            ret = (byte) (Math.abs(potential4) - 16);
        }
        return ret;
    }
}
