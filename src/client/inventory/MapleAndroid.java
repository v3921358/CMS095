...
public class MapleAndroid implements Serializable {
    ...
    public static final MapleAndroid create(final int itemid, final int uniqueid) {
        ...
        Triple<List<Integer>, List<Integer>, List<Integer>> aInfo = ii.getAndroidInfo(type);
        if (aInfo == null) {
            return null;
        }
        return create(itemid, uniqueid, aInfo.left.get(Randomizer.nextInt(aInfo.left.size())), aInfo.mid.get(Randomizer.nextInt(aInfo.mid.size())), aInfo.right.get(Randomizer.nextInt(aInfo.right.size())));
    }
