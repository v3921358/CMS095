...
public class MaplePet implements Serializable {
    ...
    public static enum PetFlag {
        ...
        PET_DRAW(0x80, 5190007, -1), //nfs
        PET_DIALOGUE(0x100, 5190008, -1), //nfs
        PET_BUFF(0x200, -1, -1),;
    ...
    public static final MaplePet loadFromDb(final int itemid, final int petid, final short inventorypos) {
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            Charset ASCII = Charset.forName(ServerConstants.ASCII);
            if (name.getBytes(ASCII).length > 13) {
                name = "宠物";
            }
            ret.setName(name);
    ...
    public final void saveToDb(Connection con) {
        ...
        try {
            final PreparedStatement ps = con.prepareStatement("UPDATE pets SET name = ?, level = ?, closeness = ?, fullness = ?, seconds = ?, flags = ?, skillid = ?, excluded = ? WHERE petid = ?");
    ...
    public static final MaplePet createPet(final int itemid, final int uniqueid) {
        return createPet(itemid, MapleItemInformationProvider.getInstance().getName(itemid), 1, 0, 100, uniqueid, itemid == 5000054 ? 18000 : 0, (short) 631/*MapleItemInformationProvider.getInstance().getPetFlagInfo(itemid)*/, 0);
    }
    ...
    public static final MaplePet createPet(int itemid, String name, int level, int closeness, int fullness, int uniqueid, int secondsLeft, short flag, int skillId) {
        ...
        String namea = name;
        Charset ASCII = Charset.forName(ServerConstants.ASCII);
        if (namea.getBytes(ASCII).length > 13) {
            namea = "宠物";
        }
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement pse = con.prepareStatement("INSERT INTO pets (petid, name, level, closeness, fullness, seconds, flags, skillid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            pse.setInt(1, uniqueid);

            pse.setString(2, namea);
