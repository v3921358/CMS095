...
public class PlayerStats implements Serializable {
    ...
    public transient double expBuff, dropBuff, mesoBuff, cashBuff, mesoGuard, mesoGuardMeso, expMod, pickupRange, dropm;
    ...
    public void recalcLocalStats(boolean first_login, MapleCharacter chra) {
        ...
        if (chra.getEventInstance() != null && chra.getEventInstance().getName().startsWith("PVP")) { //hack
            //localmaxhp = Math.min(40000, localmaxhp * 1); //3
            localmaxhp = Math.min(99999, localmaxhp * 1); //3
            localmaxmp = Math.min(20000, localmaxmp * 1);//2
        ...
        if (watk <= 0) {
            ...
            if (weapon2 != MapleWeaponType.NOT_A_WEAPON && weapon_item != null && weapon_item2 != null) {
                ...
                if (LoginServer.isLogPackets()) {
                    chra.dropMessage(-5, "副手武器: " + weapon2 + " 武器加成: " + weapon2.getMaxDamageMultiplier() + " 副手攻击: " + (mage ? we2.getMatk() : we2.getWatk()));
                }
            } else {
                if (LoginServer.isLogPackets()) {
                    chra.dropMessage(-5, "当前攻击: " + this.localmaxbasedamage + " 攻击加成: " + this.localmaxbasedamage * (this.dam_r / 100.0D));
                }
                localmaxbasedamage = weapon.getMaxDamageMultiplier() * (4 * mainstat + secondarystat) * (watk / 100.0f);
            }
            if (LoginServer.isLogPackets()) {
                chra.dropMessage(-5, "武器类型: " + weapon + " 攻击力: " + watk + " indieDamR: " + this.dam_r / 100.0D);
                chra.dropMessage(-5, "武器加成: " + weapon.getMaxDamageMultiplier() + " 主要属性: " + mainstat + " 次要属性: " + secondarystat + " 攻击力加成: " + watk / 100.0F);
                chra.dropMessage(-5, "最終攻击: " + this.localmaxbasedamage);
            }
        }
    }
    ...
    public void heal_noUpdatePvp(MapleCharacter chra) {
        setHp(getCurrentMaxHp(), chra);
        setMp(getCurrentMaxMp(), chra);
        setMaxHp(getCurrentMaxHp(), chra);
        setMaxMp(getCurrentMaxMp(), chra);
    }

    public void healPvp(MapleCharacter chra) {
        heal_noUpdatePvp(chra);
        chra.updateSingleStat(MapleStat.MAXHP, getCurrentMaxHp());
        chra.updateSingleStat(MapleStat.MAXMP, getCurrentMaxMp());
        chra.updateSingleStat(MapleStat.HP, getCurrentMaxHp());
        chra.updateSingleStat(MapleStat.MP, getCurrentMaxMp());
    }
