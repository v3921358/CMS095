...
public class PlayerHandler {
            ...
            switch (type) {
                ...
                case 3:
                    if (data <= 0) {
                        ...
                        chr.getQuestNAdd(MapleQuest.getInstance(GameConstants.BUFF_ITEM)).setCustomData(String.valueOf(data));
                    }
                    break;
            }
        }
    ...
    public static final void UseChair(final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        if (GameConstants.isFishingMap(chr.getMapId()) && (itemId == 3011000/*(itemId >= 3010001 && itemId < 4000000)*/)) {
    ...
    public static final void TrockAddMap(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (addrem == 0) {
            ...
            c.getSession().write(CSPacket.getTrockRefresh(chr, vip, true));
        } else if (addrem == 1) {
            if (!FieldLimitType.VipRock.check(chr.getMap().getFieldLimit())) {
                ...
                c.getSession().write(CSPacket.getTrockRefresh(chr, vip, false));
            } else {
...
//          AutobanManager.getInstance().addPoints(c, 1000, 60000, "Taking abnormal amounts of damge from " + monsteridfrom + ": " + damage);
        ...
        if (chr.getStat().dodgeChance > 0 && Randomizer.nextInt(100) < chr.getStat().dodgeChance) {
            //c.getSession().write(MaplePacketCreator.showSpecialEffect(GameConstants.GMS ? 20 : 22)); //lol
            return;
        }
    ...
    public static final void CancelBuffHandler(int sourceid, final MapleCharacter chr) {
        ...
        if (sourceid == 35001002) {
            if (chr.getSkillLevel(SkillFactory.getSkill(35120000)) > 0) {
                sourceid = 35120000;
            }
        }
        if (sourceid == 32001003) {
            if (chr.getSkillLevel(SkillFactory.getSkill(32120000)) > 0) {
                sourceid = 32120000;
            }
        }

        if (sourceid == 32101002) {
            if (chr.getSkillLevel(SkillFactory.getSkill(32110000)) > 0) {
                sourceid = 32110000;
            }
        }

        if (sourceid == 32101003) {
            if (chr.getSkillLevel(SkillFactory.getSkill(32120001)) > 0) {
                sourceid = 32120001;
            }
        }
    ...
    public static final void SpecialMove(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (skillid == 4121004 || skillid == 4221004) {
            if (chr.getMapId() == 100000000 || chr.getMapId() == 102000000 || chr.getMapId() == 103000000) {
                c.getSession().write(MaplePacketCreator.enableActions());
                c.getPlayer().marriage();
                return;
            }
        }
        if (skillid == 2111007 || skillid == 2211007 || skillid == 2311007 || skillid == 32111010) {
            if (chr.inPVP()) {
                chr.dropMessage(5, "PVP状态无法使用该技能。");
                c.getSession().write(MaplePacketCreator.enableActions());
                c.getPlayer().marriage();
                return;
            }
        }
        /*if (skillid == chr.getStat().getSkillByJob(1087, chr.getJob())) {
            c.getSession().write(MaplePacketCreator.enableActions());
            c.getPlayer().marriage();
            return;
        }*/

        if (skillid == chr.getStat().getSkillByJob(1085, chr.getJob())) {
            c.getSession().write(MaplePacketCreator.enableActions());
            c.getPlayer().marriage();
            return;
        }
        ...
        if (chr.getTotalSkillLevel(GameConstants.getLinkedAranSkill(skillid)) <= 0 || chr.getTotalSkillLevel(GameConstants.getLinkedAranSkill(skillid)) != skillLevel) {
            if (!GameConstants.isIceKnightSkill(skillid) && !GameConstants.isMulungSkill(skillid) && !GameConstants.isPyramidSkill(skillid) && chr.getTotalSkillLevel(GameConstants.getLinkedAranSkill(skillid)) <= 0) {
                //c.getSession().close();
                return;
            }
...
//			chr.getMap().broadcastMessage(chr, MaplePacketCreator.showMagnet(mobId, slea.readByte()), chr.getTruePosition());
    ...
    public static final void rangedAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        boolean noBullet = (chr.getJob() >= 3500 && chr.getJob() <= 3512) || GameConstants.isCannon(chr.getJob()) || GameConstants.isMercedes(chr.getJob()) || attack.skill == 3101003 || attack.skill == 3201003 || attack.skill == 15111006;
        if (attack.skill != 0) {
            skill = SkillFactory.getSkill(GameConstants.getLinkedAranSkill(attack.skill));
            if (skill == null || (GameConstants.isAngel(attack.skill) && (chr.getStat().equippedSummon % 10000) != (attack.skill % 10000))) {
                c.getSession().write(MaplePacketCreator.enableActions());
...
//		basedamage = ((effect.getMoneyCon() * 10) / 100) * effect.getProb(); // Not sure
        ...
        if (!chr.isHidden()) {
            if (attack.skill == 3211006) {
                chr.getMap().broadcastMessage(chr, MaplePacketCreator.strafeAttack(chr.getId(), attack.tbyte, attack.skill, skillLevel, attack.display, attack.speed, visProjectile, attack.allDamage, attack.position, chr.getLevel(), chr.getStat().passive_mastery(), attack.unk, chr.getTotalSkillLevel(3220010), attack.skillposition), chr.getTruePosition());
            } else {
                chr.getMap().broadcastMessage(chr, MaplePacketCreator.rangedAttack(chr.getId(), attack.tbyte, attack.skill, skillLevel, attack.display, attack.speed, visProjectile, attack.allDamage, attack.position, chr.getLevel(), chr.getStat().passive_mastery(), attack.unk, attack.skillposition), chr.getTruePosition());
            }
        } else if (attack.skill == 3211006) {
            chr.getMap().broadcastGMMessage(chr, MaplePacketCreator.strafeAttack(chr.getId(), attack.tbyte, attack.skill, skillLevel, attack.display, attack.speed, visProjectile, attack.allDamage, attack.position, chr.getLevel(), chr.getStat().passive_mastery(), attack.unk, chr.getTotalSkillLevel(3220010), attack.skillposition), false);
        } else {
            chr.getMap().broadcastGMMessage(chr, MaplePacketCreator.rangedAttack(chr.getId(), attack.tbyte, attack.skill, skillLevel, attack.display, attack.speed, visProjectile, attack.allDamage, attack.position, chr.getLevel(), chr.getStat().passive_mastery(), attack.unk, attack.skillposition), false);
        }
                    ...
                    public void run() {
                        if (!clone.isHidden()) {
                            if (attack2.skill == 3211006) {
                                clone.getMap().broadcastMessage(MaplePacketCreator.strafeAttack(clone.getId(), attack2.tbyte, attack2.skill, skillLevel2, attack2.display, attack2.speed, visProjectile2, attack2.allDamage, attack2.position, clone.getLevel(), clone.getStat().passive_mastery(), attack2.unk, chr.getTotalSkillLevel(3220010), attack2.skillposition));
                            } else {
                                clone.getMap().broadcastMessage(MaplePacketCreator.rangedAttack(clone.getId(), attack2.tbyte, attack2.skill, skillLevel2, attack2.display, attack2.speed, visProjectile2, attack2.allDamage, attack2.position, clone.getLevel(), clone.getStat().passive_mastery(), attack2.unk, attack2.skillposition));
                            }
                        } else if (attack2.skill == 3211006) {
                            clone.getMap().broadcastGMMessage(clone, MaplePacketCreator.strafeAttack(clone.getId(), attack2.tbyte, attack2.skill, skillLevel2, attack2.display, attack2.speed, visProjectile2, attack2.allDamage, attack2.position, clone.getLevel(), clone.getStat().passive_mastery(), attack2.unk, chr.getTotalSkillLevel(3220010), attack2.skillposition), false);
                        } else {
                            clone.getMap().broadcastGMMessage(clone, MaplePacketCreator.rangedAttack(clone.getId(), attack2.tbyte, attack2.skill, skillLevel2, attack2.display, attack2.speed, visProjectile2, attack2.allDamage, attack2.position, clone.getLevel(), clone.getStat().passive_mastery(), attack2.unk, attack2.skillposition), false);
                        }
...
//	slea.skip(5); // unknown
        ...
        if (res != null && c.getPlayer().getMap() != null) { // TODO more validation of input data
            if (slea.available() != 8) {
                System.out.println("角色" + chr.getName() + "(" + MapleJob.getName(MapleJob.getById(chr.getJob())) + ") slea.available != 8 (角色移動封包出錯) 剩餘封包長度: " + slea.available());
                FileoutputUtil.logToFile("logs/角色移動出錯.txt", "角色:" + chr.getName() + " 职业: " + MapleJob.getName(MapleJob.getById(chr.getJob())) + " slea.available != 8 (角色移動出錯) 封包: " + slea.toString(true));
                return;
            }
...
//	slea.readShort(); // Original Y pos
    ...
    public static void specialAttack(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (c.getPlayer().isAdmin() || LoginServer.isLogPackets()) {
            System.err.println("[SpecialAttack] - 技能ID: " + skillId + " 技能等級: " + skilllevel);
        }
        ...
        chr.getMap().broadcastMessage(chr, MaplePacketCreator.showSpecialAttack(chr.getId(), pos_x, pos_y, display, skillId, speed), chr.getTruePosition());
    }
}
