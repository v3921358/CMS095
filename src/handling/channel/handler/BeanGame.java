...
public class BeanGame {
 ...
 /*public static int[] 豆豆装备 = {1002695, 1002609, 1002665, 1002985, 1002986, 1002761, 1002760, 1002583, 1002543, 1002448, 1052137, 1092051, 1702232, 1702138};
    ...
    public static int[] 消耗品 = {2022140, 2022141, 2022139, 2022137, 1902035, 1902037};*/
        ...
        //}
        if (chr.getBeans() <= 0) {
            c.getPlayer().dropMessage(1, "你沒有小鋼珠，无法使用。");
        ...
        switch (type) {
                    ...
                    switch (第二排) {
                        ...
                        case 0://大白怪
                            if (Randomizer.nextInt(Integer.parseInt(白怪奖励[0])) > Randomizer.nextInt(Integer.parseInt(白怪奖励[1]))) {
                                ...
                                c.getPlayer().dropMessage(5, "在小鋼珠中获得：" + experi2 + "经驗值！");
            ...
            case 6:
                ...
                chr.modifyBeans(-循环次数);
                chr.setCanSetBeansNum(true);
                break;
            default:
                System.out.println("未處理未知类型【" + type + "】\n包" + slea.toString());
                //FileoutputUtil.log("logs/小钢珠未知类型.txt", "类型【" + type + "】\n包" + slea.toString());
                FileoutputUtil.logToFile("logs/小钢珠未知类型.txt", "時间: " + FileoutputUtil.NowTime() + " IP: " + chr.getClient().getSessionIPAddress() + chr.getName() + "类型【" + type + "】\n包" + slea.toString() + "\r\n");
                break;
        }
        if (type == 0x0B || type == 6) {
            for (int i = 0; i < 5; i++) {
