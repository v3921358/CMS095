...
public class PartyHandler {
            ...
            if (party != null) {
                ...
                if (action == 0x1B) { //accept
                    if (party.getMembers().size() < 6) {
                        ...
                        c.getSession().write(MaplePacketCreator.partyStatusMessage(18, null));
                    }
                } else if (action == 0x16) { // deny?
                    final MapleCharacter cfrom = c.getChannelServer().getPlayerStorage().getCharacterById(party.getLeader().getId());
                    if (cfrom != null) {
                        if (cfrom.getLevel() < 10) {
                            c.getPlayer().dropMessage(5, "对方等級低于10級，无法进行組隊。");
                        } else {
                            cfrom.getClient().getSession().write(MaplePacketCreator.partyStatusMessage(0x16, c.getPlayer().getName()));
                        }
                    }
                }
            } else {
                c.getPlayer().dropMessage(5, "已拒絕組隊邀请。");
            }
        } else {
            c.getPlayer().dropMessage(5, "不能加入組隊，因为已经有組隊了。");
        }
    ...
    public static final void PartyOperation(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (c.getPlayer().getMapId() / 10000 == 96001 || c.getPlayer().getMapId() / 10000 == 96002 || c.getPlayer().getMapId() / 10000 == 96003) {
            c.getPlayer().dropMessage(5, "当前地图不能操作組隊。");
            ...
            case 1: // create
                if (party == null) {
                    ...
                    if (partyplayer.equals(party.getLeader()) && party.getMembers().size() == 1) { //only one, reupdate
                        ...
                        c.getPlayer().dropMessage(5, "不能创建組隊，因为已经有組隊了。");
                    }
                }
                break;
            case 2: // leave
            ...
            case 4: // invite
                ...
                if (theCh > 0) {
                    ...
                    if (invited != null && invited.getParty() == null && invited.getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_INVITE)) == null) {
                        ...
                        if (party.getMembers().size() < 6) {
                            for (MaplePartyCharacter partychar : party.getMembers()) {
                                if (partychar.getName().equals(theName)) {
                                    c.getPlayer().dropMessage(5, theName + " 不能加入組隊，因为已经有組隊了。");
                                    return;
                                }
                            }
                        ...
                        c.getSession().write(MaplePacketCreator.partyStatusMessage(17, null));
                    }
                } else {
                    //c.getSession().write(MaplePacketCreator.partyStatusMessage(22, null));
                    c.getPlayer().dropMessage(5, "当前頻道未找到该角色。");
                }
                break;
            case 5: // expel
