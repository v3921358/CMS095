...
public class SummonHandler {
    ...
    public static final void MoveDragon(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        if (res != null && chr != null && chr.getDragon() != null && res.size() > 0) {
    ...
    public static void SummonAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (summon.getSkill() != 35111002 && summon.getSkill() != 35121009) {
            if (sse != null && numAttacked > sse.mobCount) {
                chr.getCheatTracker().registerOffense(CheatingOffense.召喚獸攻击怪物數量异常, " 召喚獸: " + summon.getSkill() + "  地图: " + chr.getMapId() + " 正常怪物數量: " + sse.mobCount + " 攻击怪物數量: " + numAttacked);
            }
        }
        if (summon.getSkill() != 35111002 && summon.getSkill() != 35121009) {
            if (sse != null && numAttacked > sse.mobCount) {
        ...
        for (int i = 0; i < numAttacked; i++) {
            ...
            range *= 4;
            if (summon.getPosition().distanceSq(mob.getPosition()) > range && !GameConstants.isBossMap(chr.getMapId())) {
                //chr.getCheatTracker().registerOffense(CheatingOffense.召喚獸全圖打, " 召喚獸: " + summon.getSkill() + "  地图: " + chr.getMapId() + " 預計范围: " + range + " 實際范围: " + (long) (chr.getPosition().distanceSq(mob.getPosition())) + " ");
            }
        ...
        for (Pair<Integer, Integer> attackEntry : allDamage) {
            ...
            if (chr.isGM() || toDamage < (chr.getStat().getCurrentMaxBaseDamage() * 10.0 * (summonEffect.getSelfDestruction() + summonEffect.getDamage() + chr.getStat().getDamageIncrease(summonEffect.getSourceId())) / 100.0)) { //10 x dmg.. eh
        ...
        if (!summon.isMultiAttack() || (summon.getSkill() == 35111002 && animation == 0x10)) {
