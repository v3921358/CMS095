...
function playerEntry(eim, player) {
    ...
    eim.broadcastPlayerMsg(-7, player.getName() + " 入场了。");
    ...
    if (eim.getProperty("started").equals("0")) {
        eim.broadcastPacket(UIPacket.getMidMsg("当前: " + eim.getPlayerCount() + "人/可以开始: " + i + "人", true, 1));
        eim.broadcastPacket(UIPacket.getMidMsg("目前正在招募玩家进行战斗模式。", true, 0));
        if (eim.getPlayerCount() >= (player.isGM() ? 2 : i)) {
            eim.broadcastPacket(UIPacket.clearMidMsg());
            eim.broadcastPacket(UIPacket.getMidMsg("死亡时请不要按 ESC 或者 ENTER。", true, 0));
            eim.broadcastPacket(UIPacket.getMidMsg("死亡时请不要按 ESC 或者 ENTER。", true, 0));
            eim.broadcastPacket(UIPacket.getMidMsg("死亡时请不要按 ESC 或者 ENTER。", true, 0));
            eim.broadcastPacket(MaplePacketCreator.getPVPScore(0, false));
            eim.broadcastPacket(MaplePacketCreator.getPVPMode(type));
            //eim.broadcastPacket(MaplePacketCreator.getPVPMode(11)); //PVP 1.5 EVENT!
...
function playerDead(eim, player) {
    player.getClient().getSession().write(MaplePacketCreator.getPVPKilled(player.getName() + " 已被擊敗。"));
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        eim.setProperty("ice", "0");
        eim.setProperty("red", "99999");
        eim.broadcastPlayerMsg(-7, "冰騎士被打敗了。");
        end(eim);
        return;
    }
    eim.broadcastPlayerMsg(-7, player.getName() + " 已被擊敗。");
    var type = parseInt(eim.getProperty("type"));
    if (type != 3) {
        if (player.getTeam() == 0) {
            eim.broadcastPacket(UIPacket.getMidMsg("死亡时请不要按 ESC 或者 ENTER。", true, 0));
...
function championCheck(eim) {
    ...
    if (champion != null) {
        ...
        eim.broadcastPlayerMsg(-7, champion.getName() + " 已经成为冠軍。");
...
function updateScoreboard(eim, score) {
    ...
    if (score == true || score == null) {
        ...
        for (var xx = 0; xx < players.size(); xx++) {
            if (ty != 3) {
                if (players.get(xx).getTeam() == 0) {
                    broadcastType(eim, players.get(xx));
                } else {
                    broadcastType(eim, players.get(xx));
                }
            }
