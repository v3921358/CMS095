...
function enter(pi) {
    switch (pi.getMapId()) {
        ...
        case 270040000:
            if (pi.haveItem(4032002)) {
                if(pi.isQuestActive(3522)){
                    pi.forceCompleteQuest(3522);
                    pi.playerMessage("带着远征队，去击杀品克缤BOSS把！");
                }
                if(pi.isQuestActive(3538)){
                    pi.forceCompleteQuest(3538);
                    pi.playerMessage("带着远征队，去击杀品克缤BOSS把！");
                }
                pi.playPortalSE();
                pi.warp(270040100, "out00");
                pi.playerMessage("移动到神殿废墟.");
                return true;
            } else {
                pi.playerMessage("无法繼續前进，就好像是一股力量阻止任何人进入。");
                return false;
            }
            break;
        default:
    ...
    if (pi.getQuestStatus(quest) == 2) {
        ...
        pi.playerMessage("无法挣脱时间的枷锁。被送回先前的地点。");
    }
    return true;
}