...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        //var 推广员名字 = cm.getCharacterNameById(cm.getBossRank("推广员", 2));
        ...
        if (status == 0) {
            cm.sendGetText("    Hi~#b#h ##k，这里是#bPP冒险岛#k自助系统，如果你有Cdk兑换码的话，可以在这里使用，就可以兑换了。#k\r\n\r\n请输入合法的兑换Cdk；");
        } else if (status == 1) {
            fee = cm.getText();
            ...
            if (cm.判断兑换卡类型("" + fee + "") == 1) {
                ...
                //记录玩家充值的点券
                cm.setBossRankCount("点券积分", cm.判断兑换卡数额("" + fee + "")/1000);
                //删除充值卡
				cm.Deleteexchangecard("" + fee + "");
                //判断是否有推广员，如果有，就将部分充值分享给推广员
                //if (cm.getBossRank("推广员", 2) > 0) {
                    //给推广员发送返利
                    //cm.Gaincharacterz("11340", 300, (cm.判断兑换卡数额("" + fee + "") / 100 * 推广员反馈百分比));
                    //给推广员发送小纸条
