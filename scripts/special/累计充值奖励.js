...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			var txt =""
			txt += "#e#r\t" + 心 + "\t" + 心 + "  欢迎来到PP冒险岛  " + 心 + "\t" + 心 + "\t#k#n\r\n\r\n"
			txt += "#b=================#e [累计充值奖励] #n==================#k\r\n\\r\n";
			txt += "\t#d感谢您对PP冒险岛的支持，所有充值礼包皆可获得充值积分，比例为 1 : 1\r\n\r\n";
            ...
            if (selection == 1) {	
				if (充值积分 >= 50 && cm.getPlayer().getBossLogS("赞助奖励1") == 0) {
				...
				if (充值积分 >= 100 && cm.getPlayer().getBossLogS("赞助奖励2") == 0) {
					cm.gainItemB(1112919,1,0,0,0,"",9,9,3,3,3,3,0);
					cm.sendOk("领取成功！");   
                    cm.setBossLog("赞助奖励2");
                    cm.dispose();
				} else {
				...
				if (充值积分 >= 200 && cm.getPlayer().getBossLogS("赞助奖励3") == 0) {
				...
				if (充值积分 >= 300 && cm.getPlayer().getBossLogS("赞助奖励4") == 0) {
					if (cm.getInventory(1).getItem(1) == null) {
						cm.sendOk("你的装备栏第一格没有装备。");
						cm.dispose();
						return
				...
				if (充值积分 >= 400 && cm.getPlayer().getBossLogS("赞助奖励5") == 0) {
					if (cm.getInventory(1).getItem(1) == null) {
						cm.sendOk("你的装备栏第一格没有装备。");
						cm.dispose();
						return
				...
				if (充值积分 >= 500 && cm.getPlayer().getBossLogS("赞助奖励6") == 0) {
