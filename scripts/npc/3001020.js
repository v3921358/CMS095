...
function start() {
    ...
    if (eim_status == null || eim_status.equals("0")) {
        var squadAvailability = cm.getSquadAvailability("bossmagnus");
        if (squadAvailability == -1) {
            ...
            var type = cm.isSquadLeader("bossmagnus");
            if (type == -1) {
                cm.sendOk("已经結束了申请。");
                cm.dispose();
            } else if (type == 0) {
                var memberType = cm.isSquadMember("bossmagnus");
                if (memberType == 2) {
                    cm.sendOk("在远征队的制裁名单。");
                    cm.dispose();
            ...
            if (props != null && props.equals("true")) {
                var eim = cm.getDisconnected("BossMagnus_HARD");
                if (eim == null) {
        ...
        var props = em.getProperty("leader");
        if (props != null && props.equals("true")) {
            var eimc = em.getInstance("BossMagnus_HARD");
            ...
            if ((eimc != null) && (propsc != null) && propsc.equals("1")) {
                ...
                eim = cm.getDisconnected("BossMagnus_HARD");
                if (eim == null) {
            ...
            var eimd = em.getInstance("BossMagnus_HARD");
...
function action(mode, type, selection) {
    switch (status) {
        case 0:
            if (mode == 1) {
                if (cm.getBossLogD("暴君次數") == 2) {
                    cm.sendOk("很抱歉每天只能打兩次..");
                    cm.dispose();
                    return;
                }
                if (cm.registerSquad("bossmagnus", 5, " 已经成为了远征队队长。如果你想加入远征队，请重新打开对话申请加入远征队。")) {
        ...
        case 1:
            ...
            break;
        case 2:
            if (!cm.reAdd("BossMagnus_HARD", "bossmagnus")) {
        ...
        case 3:
            if (mode == 1) {
                var squd = cm.getSquad("bossmagnus");
                if (cm.getBossLogD("暴君次數") == 2) {
                    cm.sendOk("很抱歉每天只能打兩次..");
                    cm.dispose();
                    return;
        ...
        case 5:
            if (selection == 0) { // join
                ...
                var ba = cm.addMember("bossmagnus", true);
                if (ba == 2) {
                    cm.sendOk("远征队员已经达到30名，请稍后再试。");
                } else if (ba == 1) {
                ...
                var baa = cm.addMember("bossmagnus", false);
                if (baa == 1) {
                    cm.sendOk("成功退出远征队。");
                } else {
                    cm.sendOk("你沒有參加远征队。");
                ...
                if (!cm.getSquadList("bossmagnus", 0)) {
                    cm.sendOk("由于未知的错誤，操作失敗。");
                }
            }
            cm.dispose();
        ...
        case 10:
            if (mode == 1) {
                if (selection == 0) {
                    ...
                    status = 11;
                    if (!cm.getSquadList("bossmagnus", 1)) {
                        cm.sendOk("由于未知的错誤，操作失敗。");
                        cm.dispose();
                    }
                } else if (selection == 2) {
                    status = 12;
                    if (!cm.getSquadList("bossmagnus", 2)) {
                        cm.sendOk("由于未知的错誤，操作失敗。");
                        cm.dispose();
                    }
                } else if (selection == 3) { // get insode
                    if (cm.getSquad("bossmagnus") != null) {
                        var dd = cm.getEventManager("BossMagnus_HARD");
                        dd.startInstance(cm.getSquad("bossmagnus"), cm.getMap(), 160110);
                    } else {
                        cm.sendOk("由于未知的错誤，操作失敗。");
                    }
                    cm.dispose();
            ...
            break;
        case 11:
            cm.banMember("bossmagnus", selection);
            cm.dispose();
            break;
        case 12:
            if (selection != -1) {
                cm.acceptMember("bossmagnus", selection);
            }
            cm.dispose();
            break;
        case 13:
