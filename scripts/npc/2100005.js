...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("嗨，我是#p2100005# 如果你有 #b#t5150026##k 或者 #b#t5151021##k 就可以來找我唷！ 選擇一个服務: \r\n#L0#使用:#b#t5150026##k \r\n#L1#使用:#b#t5151021##k");
		} else if (status == 1) {
			if (selection == 0) {
				beauty = 1;
				hairnew = Array();
				...
				cm.sendYesNo("你确定要使用 #b#t5150026##k #r注意:这是隨機#k");
			} else if (selection == 1) {
				beauty = 2;
				haircolor = Array();
				...
				cm.sendYesNo("你确定要使用 #b#t5151021##k #r注意:这是隨機#k ？");
			}
		}
		else if (status == 2){
			cm.dispose();
			if (beauty == 1){
				if (cm.haveItem(5150026) == true){
					...
					cm.setHair(hairnew[cm.getDoubleFloor(cm.getDoubleRandom() * hairnew.length)]);
					cm.sendOk("享受！");
				} else {
			...
			if (beauty == 2){
				if (cm.haveItem(5151021) == true){
					...
					cm.setHair(haircolor[cm.getDoubleFloor(cm.getDoubleRandom() * haircolor.length)]);
					cm.sendOk("享受！");
				} else {
