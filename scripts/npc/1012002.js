...
function action(mode, type, selection) {
    if (mode == 1)
        ...
        cm.dispose();
    if (status == 0 && mode == 1) {
        var selStr = "哈喽，我是比休斯，退休的狙擊手，但我曾经是雅典娜皮爾斯頂尖的學生，我不再打猎了，但我可以帮你制作一些对你有帮助的物品#b"
        var options = new Array("制作弓", "制作弩", "制作手套", "升級手套", "木材与螺絲釘制作", "制作箭矢");
        ...
        if (selectedType == 0) { //bow refine
            ...
            var selStr = "弩是我的專长~它的攻擊速度比弓还要慢一點，但是傷害卻比弓要來的高哦#b";
            ...
            var selStr = "升級手套? 对于弓箭手來说再適合不过了,你想要升級哪一种呢??#b";
            ...
            var selStr = "箭矢?沒问題的,1次=1000支箭矢哦#b";
            var arrows = new Array("制作箭矢", "制作弩箭矢", "制作青銅弓箭", "制作青銅弩箭", "制作鋼鐵弓箭", "制作鋼鐵弩箭");
        ...
        if (!complete) {
            cm.sendOk("身为一个弓箭手不能容許不細心的事情发生吧? 再确认你的包包是否有材料吧");
            cm.dispose();
            return;
        } else {
            if (cm.canHold(item)) {
                ...
                cm.sendOk("很棒的道具吧? 我的手藝,如果还有需要欢迎來找我,弓箭手村是我的故鄉.");
            } else {
