...
function action(mode, type, selection) {
    ...
    if(mode == 0) {
	cm.sendNext("还沒得到需要的材料吧? 但以后隨时有材料就給我吧。打猎,购买等等，得到道具的辦法有多种。");
	cm.dispose();
    }
    if(mode == 1) {
	status++;
    ...
    if(status == 0) {
	cm.sendNext("把田鼠的舌頭和雞翅混合1:1… 阿!沒有发光之粉~ 喲? 從什么时候在哪里了?");
    } else if(status == 1) {
	cm.sendSimple("我是流浪炼金术士. 还是练習工, 但我能做給你一些東西. 怎么樣? 想不想跟我交易?\r\n\r\n#b#L0#做#t4006000##l\r\n#b#L1#做#t4006001##l");
    } else if(status == 2) {
	set = selection;
	...
	cm.sendSimple("呼呼… 只有我能做#b#t"+makeitem+"##k这神秘的石頭. 最近很多冒險者说需要这个. 做#t"+makeitem+"#的辦法一共有6个. 你願意用什么辦法做?"+menu);
    } else if(status == 3) {
        //cm.playerMessage("当前選擇 status: " + status + " selection: " + selection +"  "+reqset.length);
	if (selection < 0 ) { // || selection >= reqset.length
	    //cm.playerMessage("出现错誤关閉");
	    cm.dispose();
	    return;
	}
	...
	cm.sendYesNo("为做#b5个#t"+makeitem+"##k需要如下的材料，大多是打猎怪物就会得到的。所以你努力下去，不那么难得到。怎么樣? 你真想做吗?\r\n"+menu);
    } else if(status == 4) {
	for(i=0; i < reqitem.length; i++) {
	...
	if(access == false || !cm.canHold(makeitem) || cm.getMeso() < cost) {
	    cm.sendNext("请你再确认都有需要的道具或背包的其他窗有沒有空间.");
	} else {
