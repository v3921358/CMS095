...
function action(mode, type, selection) {
    ...
    if (status == -1) {
        cm.sendSimple("你好~如果您有兴趣升級或修理你的武器，你肯定会來对地方了！我在这个偉大的神木村最好武器制造商。好了，你怎么想的武器，是充满了龙的不可思议的力量？你有兴趣吗？\r\n#L0# 做一个龙武器。\r\n#L100# 修理裝备。#l");
    } else if (status == 0) {
        if (selection == 0) {
            if (cm.haveItem(4001079)) {
                bustedDagger = true;
                cm.sendNext("这是什么？你背包里的短刀似乎坏掉了，我需要 #i" + 4011001 + "# 和 #i" + 4011002 + "#来帮你变成#i" + 4001078 + "#.");
            } else {
                var selStr = "龙的力量是不可低估的。如果你願意，我可以添加自己的權力你的武器之一。但是，武器必須足够强大，以保持其潛在的...#b";
                var options = new Array("什么是催化劑?", "做一个剑士武器", "做一个弓箭手武器", "做一个法师武器", "做一个盜賊武器", "做一个海盜武器",
                        "做一个剑士武器使用催化劑", "做一个弓箭手武器使用催化劑", "做一个法师武器使用催化劑", "做一个盜賊武器使用催化劑", "做一个海盜武器使用催化劑");
            ...
            cm.sendYesNo("太好了！我会告訴你我的能力的。大家都知道，这取决于耐久性的物品的等級和數量的物品的丟失，所以服務費各不相同？您想立即修復你的裝备？");
            status = 99;
        }
        ...
        if (bustedDagger) {
            ...
            if (selectedType == 0) { //What's a stim?
                cm.sendNext("催化劑是一种特殊的药水，我可以加入到創建某些项目的进程。它給它統計中，就好像從一个怪物下降。然而，它可能有沒有变化，而且也有可能为项低于平均水平。还有沒有得到任何项目使用刺激的时候，所以请明智的選擇有10％的機会。")
                cm.dispose();
            } else if (selectedType == 1) { //warrior weapon
                var selStr = "很好，那么你想做哪一个？？#b";
                var weapon = new Array("龙泉剑#k - 等級. 110 单手剑#b", "战龙斧#k - 等級. 110 单手斧#b", "龙頭錘#k - 等級. 110 单手錘#b", "狂龙剑#k - 等級. 110 双手剑#b", "龙王之斧#k - 等級. 110 双手斧#b", "龙之焰#k - 等級. 110 双手錘#b",
                        "幻龙长槍#k - 等級. 110 火槍#b", "赤龙长矛#k - 等級. 110 矛#b");
                ...
                var selStr = "很好，那么你想做哪一个？？#b";
                var weapon = new Array("龙形之弓#k - 等級. 110 弓#b", "飛龙弩#k - 等級. 110 弩#b");
                for (var i = 0; i < weapon.length; i++) {
                    selStr += "\r\n#L" + i + "# " + weapon[i] + "#l";
                }
                cm.sendSimple(selStr);
                ...
                var selStr = "很好，那么你想做哪一个？？#b";
                var weapon = new Array("聖龙短杖#k - 等級. 108 短杖#b", "龙骨长杖#k - 等級. 110 长杖#b");
                for (var i = 0; i < weapon.length; i++) {
                    selStr += "\r\n#L" + i + "# " + weapon[i] + "#l";
                }
                cm.sendSimple(selStr);
                ...
                var selStr = "很好，那么你想做哪一个？？#b";
                var weapon = new Array("阿拉伯彎刀#k - 等級. 110 力量短刀#b", "烈風短刃#k - 等級. 110 幸运短刀#b", "绿色龙牙拳刃#k - 等級. 110 拳套#b");
                for (var i = 0; i < weapon.length; i++) {
                    selStr += "\r\n#L" + i + "# " + weapon[i] + "#l";
                }
                cm.sendSimple(selStr);
                ...
                var selStr = "很好，那么你想做哪一个？？#b";
                var weapon = new Array("龙王之爪#k - 等級. 110 指虎#b", "聖龙金槍#k - 等級. 110 槍#b");
                for (var i = 0; i < weapon.length; i++) {
                    selStr += "\r\n#L" + i + "# " + weapon[i] + "#l";
                }
                cm.sendSimple(selStr);
        ...
        var prompt = "你想要做一个 #t" + item + "#? 在这种情況下，为了要做出好品质的裝备。请确保您有空间在您的裝备栏！#b";
        ...
        if (!complete)
            ...
            if (stimulator) { //check for stimulator
                ...
                var deleted = cm.getDoubleFloor(cm.getDoubleRandom() * 10);
                if (deleted != 0) {
                    cm.gainItem(item, 1, true)
                    cm.sendOk("完成。善待好你的武器，免得你使龙的憤怒.");
                } else {
                    cm.sendOk("不幸的是，龙的精髓...抵觸你的武器。我很抱歉是我的疏失.....");
                }
...
function getStimID(equipID) {
    var cat = cm.getDoubleFloor(equipID / 10000);
