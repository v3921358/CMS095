...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.sendSimple("欢迎來到新加坡 如果你有 #b#t5150033##k 或者 #b#t5151028 ##k? 我就可以剪你專屬的髮型\r\n#L1#使用#i5150033##t5150033##l\r\n#L2#使用#i5151028##t5151028##l");
    } else if (status == 1) {
        if (selection == 1) {
            ...
            cm.askAvatar("選擇一个喜欢的", 5150033, hair_Colo_new);
        } else if (selection == 2) {
            if (cm.getPlayer().getHair() >= 40000) {
                cm.sendOk("皇家髮型请找爱德華王子。");
                cm.dispose();
                return;
            }
            ...
            cm.askAvatar("選擇一个喜欢的", 5151028, hair_Colo_new);
        }
    } else if (status == 2) {
        if (beauty == 1) {
            if (cm.setAvatar(5150033, hair_Colo_new[selection]) == 1) {
                ...
                cm.sendOk("由于沒有#b#t5150033##k 所以我不能帮忙。");
            }
        } else {
            if (cm.setAvatar(5151028, hair_Colo_new[selection]) == 1) {
                cm.sendOk("享受！！");
            } else {
                cm.sendOk("由于沒有#b#t5151028##k 所以我不能帮忙。");
            }
        }
        cm.dispose();
    }
