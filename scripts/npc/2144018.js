...
function start() {
    ...
    if (eim_status == null || eim_status.equals("0")) {
        ...
        var props = em.getProperty("leader");
        if (props != null && props.equals("true")) {
            var eimc = em.getInstance("ArkariumBattle");
            ...
            if ((eimc != null) && (propsc != null) && propsc.equals("1")) {
                ...
                eim = cm.getDisconnected("ArkariumBattle");
                if (eim == null) {
            ...
            var eimd = em.getInstance("ArkariumBattle");
...
function action(mode, type, selection) {
    switch (status) {
        case 0:
            if (mode == 1) {
                if (cm.getBossLogD("阿卡次數") == 2) {
                    cm.sendOk("很抱歉每天只能打兩次..");
                    cm.dispose();
                    return;
                }
                if (cm.registerSquad("Arkarium", 5, " 已经成为了远征队队长。如果你想加入远征队，请重新打开对话申请加入远征队。")) {
        ...
        case 1:
            if (mode == 1) {
                var em = cm.getEventManager("ArkariumBattle");
                if (em != null) {
                    var eim = em.getInstance("ArkariumBattle");
                    if (eim != null) {
