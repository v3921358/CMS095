...
function start() {
    if (cm.getPlayer().getLevel() < 120) {
        cm.sendOk("There is a level requirement of 120 to attempt Horntail.");
        cm.dispose();
        return;
    }
    if (cm.getPlayer().getClient().getChannel() != 1 && cm.getPlayer().getClient().getChannel() != 2 && cm.getPlayer().getClient().getChannel() != 3 && cm.getPlayer().getClient().getChannel() != 4 && cm.getPlayer().getClient().getChannel() != 5 && cm.getPlayer().getClient().getChannel() != 6 && cm.getPlayer().getClient().getChannel() != 7 && cm.getPlayer().getClient().getChannel() != 8 && cm.getPlayer().getClient().getChannel() != 9 && cm.getPlayer().getClient().getChannel() != 10 && cm.getPlayer().getClient().getChannel() != 11) {
        cm.sendOk("Chaos Horntail may only be attempted on channel 10 and 11");
        cm.dispose();
        return;
    }
    ...
    if (prop == null || prop.equals("0")) {
        ...
        if (squadAvailability == -1) {
            ...
            if (time + (3 * 3600000) >= cm.getCurrentTime() && !cm.getPlayer().isGM()) {
                cm.sendOk("You have already went to Horntail in the past 12 hours. Time left: " + cm.getReadableMillis(cm.getCurrentTime(), time + (3 * 3600000)));
                cm.dispose();
                return;
            }
...
function action(mode, type, selection) {
    switch (status) {
        case 0:
            ...
            break;
        case 1:
            if (!cm.reAdd("ChaosHorntail", "Horntail")) {
                cm.sendOk("出错了，请重试.");
            }
            cm.safeDispose();
            break;
