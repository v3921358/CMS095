...
function makeCombo() {
    ...
    for (var i = 0; i < 4; i++) {
        combo += cm.getDoubleFloor(cm.getDoubleRandom() * 4) * cm.getDoublePow(10, i);
    }
...
function parsePattern(pattern) {
    ...
    for (var i = 0; i < 4; i++) {
        items[i] = cm.getDoubleFloor(tempPattern / cm.getDoublePow(10, 3 - i));
        tempPattern = tempPattern % cm.getDoublePow(10, 3 - i);
    }
    return items;
}
...
function compare(answer, guess) {
    ...
    for (var k = 0; k < answerItems.length; k++) {
        var inc = cm.getDoubleMin(answerItems[k], guessItems[k]);
...
//for mass spawn
function randX() {
    return -350 + cm.getDoubleFloor(cm.getDoubleRandom() * 750);
}