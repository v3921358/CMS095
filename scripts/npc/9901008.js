...
function start() {
	...
	status = 0;
	    if (time + (1 * 1) >= cm.getCurrentTime() && !cm.getPlayer().isGM()) {
		cm.sendOk("You have already went to Scarlion/Targa in the past 3 hours. Time left: " + cm.getReadableMillis(cm.getCurrentTime(), time + (1 * 1)));
		cm.dispose();
		return;
	    }
	cm.sendYesNo("Are you interested in becoming the leader of the Scarlion/Targa expedition Squad?");
...
function action(mode, type, selection) {
	...
	case 0:
	    if (mode == 1) {
			if (cm.registerSquad("ScarTar", 5, " 他被任命为队长。如果你想加入，请在此期间内登记加入探险队.")) {
