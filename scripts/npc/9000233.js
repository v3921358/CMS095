    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.getMap().getId() == 932200002) {
				...
				if (cm.getPlayer().getClient().getChannel() >= 2 && cm.getPlayer().getClient().getChannel() <= 10) {
                ...
                //     return;
                // }
                if (cm.getBossLogD("家族跑旗赛") >= maxPlay&&!cm.getPlayer().isGM()) {
			...
			var em = cm.getEventManager("PQS_7");
            if (selection == 0) {
                if (em.getProperty("gate") == "1") {
                    cm.warp(932200100, 0);
                    cm.setBossLog("家族跑旗赛");
                ...
                cm.sendOk("已经关闭了入口！");
                cm.worldMessage(6, "[家族跑旗赛活动] 现在管理员已经关闭了活动入口。");
                //cm.worldBrodcastEffect(5121053, "[家族活动] 现在管理员已经关闭了活动入口。");
                cm.dispose();
            } else if (selection == 4) {
                em.setProperty("gate", "1");
                cm.sendOk("已经开启入口！");
                cm.worldMessage(6, "[家族跑旗赛活动] 现在管理员已经开启了活动入口。");
		...
		var em = cm.getEventManager("PQS_7");
            ...
            em.setProperty("gate", "1");
            cm.sendOk("已经开启了入口！");
            cm.worldMessage(6, "[家族跑旗赛活动] 管理员已经开放了活动入口，请大家速度从拍卖处的副本入口进来哦！");
