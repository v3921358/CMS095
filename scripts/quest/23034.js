...
function end(d, c, b) {
	...
	if (status <= a++) {
		...
		if (qm.getJob() == 3310 && qm.getQuestStatus(23034) == 1) {

			if (status == a++) {
				qm.sendNext('你把#o9001032#破坏掉啦！呵呵，果然不出我所料！我的眼光从来都没有错。我知道你一定可以做到。这样一来，我们村的能量不足现象暂时就可以缓解了。你真的为我们村立下了大功！');
			} else {
				if (status === a++) {
					qm.sendYesNo('你的能力达到了这个程度，我就可以放心地让你进入下一阶段的课程了。虽然其他人劝我，说还很危险……但是我想你一定不会被技能难倒，一定可以成为更强的豹弩游侠！');
				} else {
					if (status === a++) {

						qm.changeJob(3311);
						qm.forceCompleteQuest(23034);
		...
		}else if(!qm.isQuestFinished(23034)){
			qm.forceStartQuest();
			qm.dispose();
		}
